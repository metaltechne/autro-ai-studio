...
export const ManufacturingOrdersView: React.FC<ManufacturingOrdersViewProps> = ({ manufacturingOrdersHook, inventory }) => {
    ...
    const handleExportPDF = async () => {
        ...
        savePdfResiliently(doc, `relatorio_ordens_fabricacao_${getTimestamp()}.pdf`);
    };
    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    {manufacturingOrders.length > 0 && (
                        <Button onClick={() => setIsConfirmingClearAll(true)} variant="danger" className="bg-red-600 hover:bg-red-700">Limpar Tudo</Button>
                    )}
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            {order.orderItems.map((item, idx) => {
                                                ...
                                                return (
                                                    <div key={`${item.componentId}-${idx}`} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-slate-300 transition-colors">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <p className="text-sm font-bold text-slate-900">
                                                                    {item.quantity}x {component?.name || (inventory.isLoading ? 'Carregando...' : `Item não encontrado (${item.componentId || 'ID ausente'})`)}
                                                                    {item.sourcingOverride === 'purchased' && (
                                                                        <span className="ml-2 text-[9px] font-black bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded uppercase tracking-widest border border-purple-200">Comprado</span>
                                                                    )}
                                                                </p>
                                    ...
                                    <div>
                                        <h4 className="text-md font-bold text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2">
                                         ...
                                         <ul className="space-y-2">
                                            {order.analysis.requirements.map((req, ridx) => (
                                                <li key={`${req.type}-${req.id}-${ridx}`} className="flex items-center gap-2 text-sm text-slate-700 bg-slate-50 p-2 rounded-lg border border-slate-100">
            ...
            <ConfirmationModal
                ...
                onConfirm={async () => {
                    await manufacturingOrdersHook.clearAllManufacturingOrders();
                    setIsConfirmingClearAll(false);
                }}
                title="Limpar Todas as Ordens de Fabricação"
                confirmText="Sim, Deletar Tudo"
                variant="danger"
            >
                <div className="space-y-3">
                    <p className="text-sm font-bold text-red-600">ATENÇÃO: Esta ação é irreversível!</p>
                    <p className="text-sm text-gray-600">Isso apagará TODAS as {manufacturingOrders.length} ordens de fabricação cadastradas no sistema. Peças em estoque não serão afetadas.</p>
                </div>
            </ConfirmationModal>
