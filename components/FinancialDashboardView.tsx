...
export const FinancialDashboardView: React.FC<FinancialDashboardViewProps> = ({ inventory, purchaseOrdersHook, manufacturingOrdersHook, manufacturing }) => {
    ...
    const kitMargins = useMemo(() => {
         const componentSkuMap = new Map<string, Component>(inventory.components.filter(c => !!c.sku).map(c => [c.sku.toUpperCase(), c]));
        ...
        return inventory.kits.map(kit => {
            ...
            kit.components.forEach(kc => {
                const component: Component | undefined = kc.componentSku ? componentSkuMap.get(kc.componentSku.toUpperCase()) : undefined;
                if (component) {
