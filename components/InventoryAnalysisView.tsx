...
export const InventoryAnalysisView: React.FC<InventoryAnalysisViewProps> = (props) => {
    ...
    const handleGenerateSummary = async () => {
        ...
        try {
            ...
            **Dados de Análise de Inventário:**
            ...
            const systemInstruction = "Você é um analista de PCP sênior da AUTRO. Analise os dados e crie um resumo executivo. foque nos Gargalos de Materiais: Identifique insumos críticos (como moedas, segredos, matéria-prima) que impedirão a produção. Seja conciso, use bullet points.";
    ...
    const handleExport = async (format: 'pdf' | 'excel') => {
        if (format === 'pdf') {
            ...
            doc.setFontSize(18);
            doc.text("Análise de Inventário e Planejamento", 200, 22, { align: 'right' });
            ...
            // Materials Tab (if active or just export it)
            if (activeTab === 'materials') {
                doc.setFontSize(14);
                doc.text("Necessidade de Insumos", 14, currentY);
                autoTable(doc, {
                    startY: currentY + 5,
                    head: [['Status', 'Insumo', 'Demanda', 'Estoque', 'Balanço']],
                    body: materialRequirements.map(m => [
                        m.status.toUpperCase(),
                        `${m.name} (${m.sku})`,
                        `${m.projectedDemand} ${m.unit}`,
                ...
                currentY = (doc as any).lastAutoTable.finalY + 15;
                 ...
                 doc.setFontSize(14);
                 doc.text("Curva ABC de Componentes", 14, currentY);
                 autoTable(doc, {
                     startY: currentY + 5,
                     head: [['Classe', 'Componente', 'Valor Consumo', '% Acum']],
                     body: abcAnalysis.map(i => [
                         i.classification,
                         i.name,
                         formatCurrency(i.consumptionValue),
                 ...
                 doc.setFontSize(14);
                 doc.text("Alertas de Reposição", 14, currentY);
                 autoTable(doc, {
                     startY: currentY + 5,
                     head: [['Item', 'Estoque', 'Ponto Pedido', 'Sugerido']],
                     body: reorderPointAlerts.map(i => [
                         i.name,
                         i.currentStock,
                         i.reorderPoint,
                         i.suggestedOrderQty
            ...
            savePdfResiliently(doc, `analise_inventario_${getTimestamp()}.pdf`);
        } else {
            const wb = XLSX.utils.book_new();
            
            const materialsWS = XLSX.utils.json_to_sheet(materialRequirements);
            XLSX.utils.book_append_sheet(wb, materialsWS, 'Materiais');

            const abcWS = XLSX.utils.json_to_sheet(abcAnalysis);
            XLSX.utils.book_append_sheet(wb, abcWS, 'ABC');

            const reorderWS = XLSX.utils.json_to_sheet(reorderPointAlerts);
            XLSX.utils.book_append_sheet(wb, reorderWS, 'Reposição');

            XLSX.writeFile(wb, `analise_inventario_${getTimestamp()}.xlsx`);
        }
    };
            ...
            <Card className="mb-6 print-hide">
                <div className="flex wrap items-end gap-4">
                    ...
                    <div className="flex-shrink-0 flex items-center gap-2">
                        <Button onClick={() => handleExport('pdf')} variant="secondary">PDF</Button>
                        <Button onClick={() => handleExport('excel')} variant="secondary">Excel</Button>
                                        ...
                                        {materialRequirements.map(item => (
                                            <tr key={item.componentId} className={item.status === 'critical' ? 'bg-red-50' : item.status === 'warning' ? 'bg-yellow-50' : ''}>
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-black">
                                                    ...
                                                    {item.familiaName && <div className="text-[10px] text-indigo-500 font-bold uppercase mt-0.5">{item.familiaName}</div>}
                                                </td>
