...
export const KitsView: React.FC<KitsViewProps> = ({ inventory, manufacturing, onShowQRCode, onViewDetails }) => {
  ...
  const [globalFastenerFamiliaId, setGlobalFastenerFamiliaId] = useState<string>('');
  const [globalNutFamiliaId, setGlobalNutFamiliaId] = useState<string>('');
  const [tempFastenerId, setTempFastenerId] = useState<string>('');
  const [tempNutId, setTempNutId] = useState<string>('');
  ...
  const paginatedKits = filteredKits.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
    ...
    // Helper to aggregate breakdown items by SKU
    const addToBreakdownMap = (map: Map<string, KitCostBreakdownItem>, item: KitCostBreakdownItem) => {
    ...
    for (const kit of visibleKits) {
        try {
            const kitFixFamiliaId = kit.selectedFamiliaId || globalFastenerFamiliaId;
            const kitNutFamiliaId = kit.selectedNutFamiliaId || globalNutFamiliaId;

            let baseTotalCost = 0;
            let baseMaterialCost = 0;
            let baseFabricationCost = 0;
            const aggregatedBase = new Map<string, KitCostBreakdownItem>();
            
            if (kit.components && Array.isArray(kit.components)) {
                kit.components.forEach((kc: KitComponent) => {
                    if (!kc.componentSku) return;
                    const component: Component | undefined = findComponent(kc.componentSku);
                    if (component) {
                        ...
                        addToBreakdownMap(aggregatedBase, {
        ...
        const aggregatedDefault = new Map<string, KitCostBreakdownItem>();
        Array.from(aggregatedBase.values()).forEach(v => aggregatedDefault.set(v.sku, { ...v }));
        ...
        if (kit.requiredFasteners && Array.isArray(kit.requiredFasteners)) {
            kit.requiredFasteners.forEach((rf: { dimension: string; quantity: number }) => {
                ...
                addToBreakdownMap(aggregatedDefault, {
                    name, sku: `${familyTag}-${rf.dimension.toUpperCase().replace('MM', '')}`, quantity: rf.quantity,
                    unitCost, 
                ...
                if (!isNut) {
                    ...
                    if (!isNaN(bitola)) {
                            ...
                            if (copoFamilia) {
                                const res = evaluateProcess(copoFamilia, { bitola: copoBitola }, inventory.components, {}, { allFamilias: manufacturing.familias, workStations: manufacturing.workStations, consumables: manufacturing.consumables, operations: manufacturing.standardOperations });
                                const unitCost = res.custoMateriaPrima + res.custoFabricacao;
                                const cupTotal = unitCost * rf.quantity;
                                
                                defaultTotalCost += cupTotal;
                                defaultMaterialCost += res.custoMateriaPrima * rf.quantity;
                                defaultFabricationCost += res.custoFabricacao * rf.quantity;
        ...
        const defaultBreakdown = Array.from(aggregatedDefault.values());
        ...
        // Get identifying family names for default view
        let defaultFastenerName = '';
        let defaultNutName = '';
        
        if (kitFixFamiliaId) {
            defaultFastenerName = kitFixFamiliaId === 'fixS' ? 'FIX-S' : (kitFixFamiliaId === 'fixP' ? 'FIX-P' : (manufacturing.familias.find(f => f.id === kitFixFamiliaId)?.nome || kitFixFamiliaId));
        }
        if (kitNutFamiliaId) {
            defaultNutName = kitNutFamiliaId === 'porP' ? 'POR-P' : (manufacturing.familias.find(f => f.id === kitNutFamiliaId)?.nome || kitNutFamiliaId);
        }

        let fixSTotalCost = baseTotalCost;
        let fixSMaterialCost = baseMaterialCost;
        let fixSFabricationCost = baseFabricationCost;
        const aggregatedFixS = new Map<string, KitCostBreakdownItem>();
        Array.from(aggregatedBase.values()).forEach(v => aggregatedFixS.set(v.sku, { ...v }));
        ...
        if (kit.requiredFasteners) {
            kit.requiredFasteners.forEach((rf: { dimension: string; quantity: number }) => {
                ...
                addToBreakdownMap(aggregatedFixS, {
                ...
                if (!isNut) {
                    ...
                    if (!isNaN(bitola)) {
                        ...
                        if (copoFamilia) {
                            const res = evaluateProcess(copoFamilia, { bitola: copoBitola }, inventory.components, {}, { allFamilias: manufacturing.familias, workStations: manufacturing.workStations, consumables: manufacturing.consumables, operations: manufacturing.standardOperations });
                            const cupTotal = (res.custoMateriaPrima + res.custoFabricacao) * rf.quantity;
                            fixSTotalCost += cupTotal;
                            fixSMaterialCost += res.custoMateriaPrima * rf.quantity;
                            fixSFabricationCost += res.custoFabricacao * rf.quantity;
                            addToBreakdownMap(aggregatedFixS, {
                                name: componentFound ? componentFound.name : `Copo ${formattedBitola.replace('.', ',')}`, 
                                sku: componentFound ? componentFound.sku : skuToFind, quantity: rf.quantity,
                                unitCost: res.custoMateriaPrima + res.custoFabricacao,
                                materialCost: res.custoMateriaPrima,
                                fabricationCost: res.custoFabricacao,
                                totalCost: cupTotal, type: 'Copo',
                                costBreakdown: res.costBreakdown
        ...
        const fixSBreakdown = Array.from(aggregatedFixS.values());

        let fixPTotalCost = baseTotalCost;
        let fixPMaterialCost = baseMaterialCost;
        let fixPFabricationCost = baseFabricationCost;
        const aggregatedFixP = new Map<string, KitCostBreakdownItem>();
        Array.from(aggregatedBase.values()).forEach(v => aggregatedFixP.set(v.sku, { ...v }));
        ...
        if (kit.requiredFasteners) {
            kit.requiredFasteners.forEach((rf: { dimension: string; quantity: number }) => {
                ...
                addToBreakdownMap(aggregatedFixP, {
                    name: isNut ? `Porca M${rf.dimension.split('x')[0]}` : `Fixador ${rf.dimension}`,
                    sku: `${familyTag}-${rf.dimension.toUpperCase().replace('MM', '')}`, quantity: rf.quantity,
                    unitCost: fastenerDetails.unitCost, 
                    materialCost: fastenerDetails.materialCost,
                ...
                if (!isNut) {
                    ...
                    if (!isNaN(bitola)) {
                        ...
                        if (copoFamilia) {
                            const res = evaluateProcess(copoFamilia, { bitola: copoBitola }, inventory.components, {}, { allFamilias: manufacturing.familias, workStations: manufacturing.workStations, consumables: manufacturing.consumables, operations: manufacturing.standardOperations });
                            const cupTotal = (res.custoMateriaPrima + res.custoFabricacao) * rf.quantity;
                            fixPTotalCost += cupTotal;
                            fixPMaterialCost += res.custoMateriaPrima * rf.quantity;
                            fixPFabricationCost += res.custoFabricacao * rf.quantity;
                            addToBreakdownMap(aggregatedFixP, {
                                name: componentFound ? componentFound.name : `Copo ${formattedBitola.replace('.', ',')}`, 
                                sku: componentFound ? componentFound.sku : skuToFind, quantity: rf.quantity,
                                unitCost: res.custoMateriaPrima + res.custoFabricacao,
        ...
        const fixPBreakdown = Array.from(aggregatedFixP.values());
        ...
        // Ensure nut family name is also in options if present
        if (defaultNutName) {
            if (options.fixS) options.fixS.selectedNutFamiliaName = defaultNutName;
            if (options.fixP) options.fixP.selectedNutFamiliaName = defaultNutName;
        }
        ...
        detailsMap.set(kit.id, { 
            ...
            saleDetails,
            label: 'Padrão',
            selectedFastenerFamiliaName: defaultFastenerName || 'AUTO (FIX-S)',
            selectedNutFamiliaName: defaultNutName || 'AUTO (POR-P)',
            options
        });
        } catch (err) {
    ...
    return detailsMap;
  }, [paginatedKits, components, manufacturing.familias, manufacturing.workStations, manufacturing.consumables, manufacturing.standardOperations, inventory.components, calculateSaleDetails, globalFastenerFamiliaId, globalNutFamiliaId]);
