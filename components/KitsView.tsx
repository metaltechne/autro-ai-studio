...
import { InventoryHook, Kit, ManufacturingHook, KitCostDetails, KitCostBreakdownItem, ScannedQRCodeData, SaleDetails, Component, View, KitComponent, SaleItem, CostStep, FamiliaComponente } from '../types';
...
const KitCard: React.FC<{ 
    ...
    manufacturing: ManufacturingHook;
}> = ({ kit, costDetails, onViewDetails, onShowSaleDetails, onPrint, calculateSaleDetails, manufacturing }) => {
    const [selectedOption, setSelectedOption] = useState<string>('default');
    const { canViewCosts } = usePermissions();

    const familiesInfo = useMemo(() => {
        const hasFasteners = kit.requiredFasteners?.some(f => !f.dimension.includes('x0'));
        const hasNuts = kit.requiredFasteners?.some(f => f.dimension.includes('x0'));

        const fixFam = (hasFasteners && kit.selectedFamiliaId) ? manufacturing.familias.find(f => f.id === kit.selectedFamiliaId) : null;
        const nutFam = (hasNuts && kit.selectedNutFamiliaId) ? manufacturing.familias.find(f => f.id === kit.selectedNutFamiliaId) : null;
        return { fixFam, nutFam };
    }, [kit.selectedFamiliaId, kit.selectedNutFamiliaId, kit.requiredFasteners, manufacturing.familias]);

    const visibleOptions = useMemo(() => {
        if (!costDetails.options) return [];
        return Object.keys(costDetails.options).filter(optKey => {
            const label = optKey === 'fixS' ? 'FIX-S' : 'FIX-P';
            const famSelected = familiesInfo.fixFam;
            if (famSelected && (famSelected.nome?.toUpperCase().includes(label) || famSelected.id.toUpperCase().includes(label))) return false;
            return true;
        });
    }, [costDetails.options, familiesInfo.fixFam]);
        ...
        <Card className="flex flex-col h-full transition-all duration-300 hover:shadow-float border-none shadow-soft p-0 overflow-hidden bg-white group cursor-pointer" onClick={() => onViewDetails(kit.id)}>
            <div className="h-32 bg-autro-blue relative flex items-center justify-center overflow-hidden">
                ...
                <div className="absolute top-3 right-3 flex flex-col gap-1 items-end">
                    ...
                    {familiesInfo.fixFam && (
                        <div className="bg-emerald-500/80 backdrop-blur-sm px-2 py-0.5 rounded text-[8px] font-black text-white uppercase tracking-widest shadow-sm">
                            FIX: {familiesInfo.fixFam.nome}
                        </div>
                    )}
                    {familiesInfo.nutFam && (
                        <div className="bg-amber-500/80 backdrop-blur-sm px-2 py-0.5 rounded text-[8px] font-black text-white uppercase tracking-widest shadow-sm">
                            POR: {familiesInfo.nutFam.nome}
                        </div>
                    )}
                </div>
            </div>
                    ...
                    <div className="flex flex-col space-y-2">
                        ...
                        {visibleOptions.length > 0 && (
                            <div className="flex gap-1 overflow-x-auto pb-1 no-scrollbar" onClick={(e) => e.stopPropagation()}>
                                ...
                                {visibleOptions.map(optKey => (
                                    <button 
                                        key={optKey}
                                        onClick={() => setSelectedOption(optKey)}
                                        className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter transition-all shadow-sm ${
                                            selectedOption === optKey 
                                                ? (optKey === 'fixS' ? 'bg-indigo-600 text-white' : 'bg-amber-600 text-white') 
                                                : 'bg-white text-slate-400 border border-slate-200 hover:border-slate-400'
                                        }`}
                                    >
                                        Simular {optKey === 'fixS' ? 'FIX-S' : 'FIX-P'}
                                    </button>
                                ))}
                            </div>
                        )}
...
export const KitsView: React.FC<KitsViewProps> = ({ inventory, manufacturing, onShowQRCode, onViewDetails }) => {
  ...
  const [bulkNutFamilyId, setBulkNutFamilyId] = useState('');
  const [isApplyingFamily, setIsApplyingFamily] = useState(false);
  ...
  const groupedFamilies = useMemo(() => {
    ...
    generatorFamilias.forEach(f => {
        ...
        // Prioridade 1: Tags específicas se existirem
        const tag = f.masterProcessTag?.toUpperCase();
        if (tag && groups[tag]) {
            groups[tag].push(f);
            return;
        }

        // Prioridade 2: Busca por padrões no nome
        if (nome.includes('FIX-S EXT') || nome.includes('FIX S EXT')) {
            groups['FIX-S EXT'].push(f);
        } else if (nome.includes('FIX-S') || nome.includes('FIX S')) {
            groups['FIX-S'].push(f);
        } else if (nome.includes('FIX-P') || nome.includes('FIX P')) {
            groups['FIX-P'].push(f);
        } else if (nome.includes('PORCA') || nome.includes('POR-P') || nome.includes('POR P')) {
            groups['PORCAS'].push(f);
        } else {
            groups['OUTRAS FAMÍLIAS'].push(f);
        }
    });
  ...
  useEffect(() => {
    ...
    // Improved family detection with more specific patterns
    ...
    const getFastenerDetails = (dimension: string, familia: any): { unitCost: number, materialCost: number, fabricationCost: number, breakdown: CostStep[] } => {
      ...
      if (!isNaN(bitola)) {
          const result = evaluateProcess(
              ...
              {},
              { allFamilias: manufacturing.familias, workStations: manufacturing.workStations, consumables: manufacturing.consumables, operations: manufacturing.standardOperations }
          );
          return {
              unitCost: result.custoMateriaPrima + result.custoFabricacao,
              materialCost: result.custoMateriaPrima,
              fabricationCost: result.custoFabricacao,
    ...
    for (const kit of (kits || [])) {
            ...
            if (kit.components && Array.isArray(kit.components)) {
                kit.components.forEach((kc: KitComponent) => {
                    ...
                    if (component) {
                        ...
                        let itemBreakdown: CostStep[] = [];
                        if (component.type === 'raw_material' || component.sourcing === 'purchased') {
                            baseMaterialCost += itemTotalCost;
                        } else {
                            const matSub = (component.custoMateriaPrima || 0) * kc.quantity;
                            const fabSub = (component.custoFabricacao || 0) * kc.quantity;
                            baseMaterialCost += matSub;
                            baseFabricationCost += fabSub;

                            // Get breakdown for manufactured component
                            if (component.familiaId) {
                                const familia = manufacturing.familias.find(f => f.id === component.familiaId);
                                if (familia) {
                                    const res = evaluateProcess(familia, {}, inventory.components, {}, { allFamilias: manufacturing.familias, workStations: manufacturing.workStations, consumables: manufacturing.consumables, operations: manufacturing.standardOperations });
                                    itemBreakdown = res.costBreakdown;
                                }
                            }
                        }
                        ...
                        breakdown.push({
                            ...
                            unitCost: unitCost, 
                            materialCost: component.type === 'raw_material' || component.sourcing === 'purchased' ? unitCost : (component.custoMateriaPrima || 0),
                            fabricationCost: component.type === 'raw_material' || component.sourcing === 'purchased' ? 0 : (component.custoFabricacao || 0),
                            totalCost: itemTotalCost, type: 'Componente',
                            costBreakdown: itemBreakdown
                        });
                    }
                });
            }
        ...
        if (kit.requiredFasteners && Array.isArray(kit.requiredFasteners)) {
            kit.requiredFasteners.forEach((rf: { dimension: string; quantity: number }) => {
                ...
                const unitCost = fastenerDetails.unitCost;
                const itemTotalCost = unitCost * rf.quantity;
                
                defaultTotalCost += itemTotalCost;
                defaultMaterialCost += fastenerDetails.materialCost * rf.quantity;
                defaultFabricationCost += fastenerDetails.fabricationCost * rf.quantity;
                ...
                defaultBreakdown.push({
                    ...
                    unitCost, 
                    materialCost: fastenerDetails.materialCost,
                    fabricationCost: fastenerDetails.fabricationCost,
                    totalCost: itemTotalCost, type: 'Fixador',
                    costBreakdown: fastenerDetails.breakdown
                });
            });
        }
        ...
        let fixSFabricationCost = baseFabricationCost;
        ...
        if (kit.requiredFasteners) {
            kit.requiredFasteners.forEach((rf: { dimension: string; quantity: number }) => {
                ...
                const itemTotalCost = fastenerDetails.unitCost * rf.quantity;
                fixSTotalCost += itemTotalCost;
                fixSMaterialCost += fastenerDetails.materialCost * rf.quantity;
                fixSFabricationCost += fastenerDetails.fabricationCost * rf.quantity;

                fixSBreakdown.push({
                    name: isNut ? `Porca M${rf.dimension.split('x')[0]}` : `Fixador ${rf.dimension}`,
                    sku: `DIM-${rf.dimension}`, quantity: rf.quantity,
                    unitCost: fastenerDetails.unitCost, 
                    materialCost: fastenerDetails.materialCost,
                    fabricationCost: fastenerDetails.fabricationCost,
                    totalCost: itemTotalCost, type: 'Fixador',
                    costBreakdown: fastenerDetails.breakdown
        ...
        let fixPFabricationCost = baseFabricationCost;
        ...
        if (kit.requiredFasteners) {
            kit.requiredFasteners.forEach((rf: { dimension: string; quantity: number }) => {
                ...
                fixPMaterialCost += fastenerDetails.materialCost * rf.quantity;
                fixPFabricationCost += fastenerDetails.fabricationCost * rf.quantity;
        ...
        if (fixSFamilia) {
            options.fixS = {
                ...
                fabricationCost: fixSFabricationCost,
        ...
        if (fixPFamilia) {
            options.fixP = {
                ...
                fabricationCost: fixPFabricationCost,
    ...
    return detailsMap;
  }, [kits, components, manufacturing.familias, manufacturing.workStations, manufacturing.consumables, manufacturing.standardOperations, inventory.components, calculateSaleDetails]);
  ...
  const handlePrintTechnicalSheet = async (kit: Kit, detailsOverride?: KitCostDetails) => {
    ...
    doc.text(`${kit.name}${variationLabel}`, 14, 42);
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Identificação (SKU): ${kit.sku}`, 14, 48);
    doc.text(`Marca: ${kit.marca} | Modelo: ${kit.modelo} | Ano: ${kit.ano}`, 14, 53);
    ...
    if (canViewCosts) {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text(`Custo Matéria-Prima: ${formatCurrency(details.materialCost || 0)}`, 14, 72);
        doc.text(`Custo de Fabricação: ${formatCurrency(details.fabricationCost || 0)}`, 14, 77);
        doc.setFont('helvetica', 'bold');
        doc.text(`Custo Total Produção: ${formatCurrency(details.totalCost)}`, 14, 82);
        doc.setFont('helvetica', 'normal');
        doc.text(`Preço de Venda: ${formatCurrency(details.saleDetails?.sellingPrice)}`, 120, 72);
        doc.text(`Margem de C.: ${formatCurrency(details.saleDetails?.contributionMargin)} (${details.saleDetails?.contributionMarginPercentage.toFixed(1)}%)`, 120, 77);
    }

    // Breakdown Table
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('COMPOSIÇÃO E DETALHAMENTO DE CUSTOS', 14, 95);
  ...
  const handleDetailedExportExcel = () => {
    ...
    // Step 1: Gather and summarize all data into a flat structure (one row per kit/variant)
    filteredKits.forEach(kit => {
        const details = kitCostDetailsMap.get(kit.id);
        if (!details) return;

        const variants = [{ label: 'Padrão', details: details }];
        if (details.options) {
            Object.entries(details.options).forEach(([key, opt]) => {
                variants.push({ label: (opt as any).label || key, details: opt as any });
            });
        }
        ...
        variants.forEach(v => {
            const row: any = {
                ...
                'Modelo': kit.modelo,
                'Ano': kit.ano,
                'Ficha Técnica (Fixadores)': kit.requiredFasteners?.map(f => `${f.quantity}x ${f.dimension}`).join('; ') || '',
            };

            // Global aggregators for the row
            let totalMat = 0;
            let totalMOA = 0;
            let totalINS = 0;
            let totalTime = 0;

            // Map components and their internal processes to columns
            v.details.breakdown.forEach((item: any) => {
                // Column for the component itself
                const compKey = `COMP: ${item.name}`;
                row[compKey] = (row[compKey] || 0) + item.totalCost;

                // Decompose internal processes into aggregated columns by name
                (item.costBreakdown || []).forEach((step: any) => {
                    const stepCost = step.cost * item.quantity;
                    const procKey = `PROC: ${step.name.toUpperCase()}`;
                    
                    // Accumulate cost in a specific column for this process name
                    row[procKey] = (row[procKey] || 0) + stepCost;

                    // Category buckets
                    if (step.type === 'material') totalMat += stepCost;
                    else if (step.type === 'labor') totalMOA += stepCost;
                    else if (step.type === 'consumable' || step.name.toLowerCase().includes('insumo')) totalINS += stepCost;

                    if (step.timeSeconds) totalTime += (step.timeSeconds * item.quantity);
                });
            });

            // Summary Data
            row['TOTAL MATÉRIA-PRIMA (R$)'] = totalMat;
            row['TOTAL MÃO-DE-OBRA (R$)'] = totalMOA;
            row['TOTAL INSUMOS (R$)'] = totalINS;
            row['TEMPO TOTAL (min)'] = (totalTime / 60).toFixed(2);
            row['CUSTO TOTAL PRODUÇÃO (R$)'] = v.details.totalCost;
            row['PREÇO VENDA SUGERIDO (R$)'] = v.details.saleDetails.sellingPrice;
            row['MARGEM CONTRIBUIÇÃO (R$)'] = v.details.saleDetails.contributionMargin;
            row['MARGEM (%)'] = (v.details.saleDetails.contributionMarginPercentage).toFixed(1) + '%';
            row['ID KIT'] = kit.id;
            row['LINHA FIXADORES'] = kit.selectedFamiliaId || 'N/A';
            row['LINHA PORCAS'] = kit.selectedNutFamiliaId || 'N/A';
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Resumo Estruturado de Custos');
    
    // Auto-size columns slightly better
    const wscols = dataToExport.length > 0 ? Object.keys(dataToExport[0]).map(() => ({ wch: 20 })) : [];
    if (wscols.length > 0) {
        wscols[0].wch = 35; // Kit name
        wscols[6].wch = 40; // Fixadores
    }
    worksheet['!cols'] = wscols;

    XLSX.writeFile(workbook, `ficha_tecnica_unificada_kits_${Date.now()}.xlsx`);
  };

  const handleApplyFamilyToFilteredKits = async () => {
    if ((!bulkFamilyId && !bulkNutFamilyId) || filteredKits.length === 0) return;
    
    setIsApplyingFamily(true);
    try {
        const kitsToUpdate = filteredKits.map(k => {
            const updated = { ...k };
            if (bulkFamilyId) updated.selectedFamiliaId = bulkFamilyId;
            if (bulkNutFamilyId) updated.selectedNutFamiliaId = bulkNutFamilyId;
            return updated;
        });
        
        await inventory.updateMultipleKits(kitsToUpdate);
        inventory.refreshFromCloud(); // Force refresh to show changes
        
        setBulkFamilyId('');
        setBulkNutFamilyId('');
    } catch (error) {
        console.error("Error applying family:", error);
    } finally {
        setIsApplyingFamily(false);
    }
  };
        ...
        <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200">
                <Select 
                    ...
                    className="w-40 h-10 text-[9px] font-bold uppercase tracking-wider"
                >
                    <option value="">LINHA FIX...</option>
                    {Object.entries(groupedFamilies).map(([tag, families]) => (
                        families.length > 0 && (
                            <optgroup key={`bulk-fix-${tag}`} label={tag}>
                                {families.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}
                            </optgroup>
                        )
                    ))}
                </Select>
                <Select 
                    value={bulkNutFamilyId} 
                    onChange={(e) => setBulkNutFamilyId(e.target.value)} 
                    className="w-40 h-10 text-[9px] font-bold uppercase tracking-wider"
                >
                    <option value="">LINHA POR...</option>
                    {Object.entries(groupedFamilies).map(([tag, families]) => (
                        families.length > 0 && (
                            <optgroup key={`bulk-por-${tag}`} label={tag}>
                                {families.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}
                            </optgroup>
                        )
                    ))}
                ...
                <Button 
                    onClick={handleApplyFamilyToFilteredKits} 
                    disabled={(!bulkFamilyId && !bulkNutFamilyId) || filteredKits.length === 0 || isApplyingFamily}
                    className="bg-slate-900 h-10 text-[10px] uppercase font-black"
                >
                    {isApplyingFamily ? 'Aplicando...' : 'Aplicar'}
                </Button>
            </div>
            <div className="flex gap-2">
                {isDirty && (
                ...
                <Button onClick={handleExportExcel} className="bg-emerald-600 hover:bg-emerald-700 text-white" disabled={filteredKits.length === 0}>Exportar Excel (Ficha Técnica)</Button>
