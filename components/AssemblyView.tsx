...
export const AssemblyView: React.FC<AssemblyViewProps> = ({ inventory, manufacturing, productionOrdersHook, cuttingOrdersHook, manufacturingOrdersHook, purchaseOrdersHook, setCurrentView }) => {
  ...
  const handleAddComponent = (kit: Kit, quantity: number = 1) => {
    setOrder(prev => {
        const existing = prev.kits.find(item => item.kitId === kit.id);
        if (existing) return { ...prev, kits: prev.kits.map(i => i.kitId === kit.id ? { ...i, quantity: i.quantity + quantity } : i)};
        return { ...prev, kits: [...prev.kits, { kitId: kit.id, quantity: quantity }]};
    });
    addToast(`${kit.name} adicionado ao lote.`, 'info');
  };
                    ...
                    <div className="lg:col-span-8 flex flex-col min-h-0">
                        <div className="flex-grow overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                            {order.kits.map((item) => {
                                ...
                                return (
                                    <div key={item.kitId} className="p-4 bg-white rounded-2xl border-2 border-slate-100 shadow-sm flex items-center justify-between group hover:border-blue-200 transition-all">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">SKU: {kit?.sku}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-6">
                                            ...
                                            <button onClick={() => setOrder(prev => ({...prev, kits: prev.kits.filter(i => i.kitId !== item.kitId)}))} className="p-2 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 gap-3">
                                            {selectedKitForDetail.components.map((comp, idx) => {
                                                const componentData = components.find(c => c.id === comp.componentId);
                                                return (
                                                    <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">SKU: {componentData?.sku}</p>
                                                            </div>
                                                        </div>
                    ...
                    <div className="p-8 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6">
                        ...
                        <Button 
                            onClick={() => {
                                ...
                                handleAddComponent(selectedKitForDetail, qty);
