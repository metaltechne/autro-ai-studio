...
export const AssemblyView: React.FC<AssemblyViewProps> = ({ inventory, manufacturing, productionOrdersHook, cuttingOrdersHook, manufacturingOrdersHook, purchaseOrdersHook, setCurrentView }) => {
  ...
  const handleAddComponent = (kit: Kit, quantity: number = 1, variant: 'Padrão' | 'Fix-S' | 'Fix-P' = 'Padrão') => {
    setOrder(prev => {
        const existing = prev.kits.find(item => item.kitId === kit.id && item.variant === variant);
        if (existing) return { ...prev, kits: prev.kits.map(i => (i.kitId === kit.id && i.variant === variant) ? { ...i, quantity: i.quantity + quantity } : i)};
        return { ...prev, kits: [...prev.kits, { kitId: kit.id, quantity: quantity, variant }]};
    });
    addToast(`${kit.name} (${variant}) adicionado ao lote.`, 'info');
  };
                    ...
                    <div className="flex-grow overflow-y-auto pr-1 custom-scrollbar">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 pb-10">
                            {filteredKits.map(kit => {
                                const quantityInWorkshop = order.kits.find(item => item.kitId === kit.id)?.quantity || 0;
                                return (
                                    <div 
                                        key={kit.id} 
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-50">
                                            ...
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleAddComponent(kit); }}
                ...
                <div className="h-full grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fade-in overflow-hidden">
                        ...
                        <Card className="p-6 border-2 border-slate-900 bg-slate-900 text-white rounded-3xl shadow-2xl">
                            ...
                            <div className="space-y-4">
                                <Select 
                                    ...
                                    className="!bg-white/10 !text-white !border-white/20 !h-12 text-xs"
                                >
                                ...
                                <Input 
                                    ...
                                    className="!bg-white/10 !text-white !border-white/20 !h-12 !placeholder-white/30 font-bold text-xs"
                                />
                            </div>
                        </Card>
                        ...
                        <div className="p-6 bg-blue-50 border-2 border-blue-100 rounded-3xl">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-500 text-[10px] font-bold uppercase">Total de Kits</span>
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-blue-900 font-black text-xl">{order.kits.length}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-8 flex flex-col min-h-0">
                        <div className="flex-grow overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                            {order.kits.map((item, index) => {
                                const kit = findKitById(item.kitId);
                                return (
                                    <div key={`${item.kitId}-${item.variant}-${index}`} className="p-4 bg-white rounded-2xl border-2 border-slate-100 shadow-sm flex items-center justify-between group hover:border-blue-200 transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-slate-300 font-black text-lg border border-slate-100">
                                                {kit?.name.charAt(0)}
                                            </div>
                                            <div>
                                                <p className="font-black text-slate-900 text-sm uppercase leading-tight">{kit?.name}</p>
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">SKU: {kit?.sku} • Variante: {item.variant || 'Padrão'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-6">
                                            <div className="flex items-center gap-3 bg-slate-100 p-1 rounded-xl border border-slate-200">
                                                <button onClick={() => {
                                                    setOrder(prev => ({...prev, kits: prev.kits.map(i => (i.kitId === item.kitId && i.variant === item.variant) ? {...i, quantity: Math.max(1, i.quantity - 1)} : i)}));
                                                ...
                                                <button onClick={() => {
                                                    setOrder(prev => ({...prev, kits: prev.kits.map(i => (i.kitId === item.kitId && i.variant === item.variant) ? {...i, quantity: i.quantity + 1} : i)}));
                                                }} className="w-8 h-8 flex items-center justify-center rounded-lg bg-white text-slate-900 font-black shadow-sm hover:bg-slate-50 transition-colors">+</button>
                                            </div>
                                            <button onClick={() => setOrder(prev => ({...prev, kits: prev.kits.filter(i => !(i.kitId === item.kitId && i.variant === item.variant))}))} className="p-2 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all">
                    ...
                    <div className="p-8 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6">
                        ...
                        <Button 
                            onClick={() => {
                                ...
                                handleAddComponent(selectedKitForDetail, qty, variant);
