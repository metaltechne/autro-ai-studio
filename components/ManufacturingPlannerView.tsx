...
export const ManufacturingPlannerView: React.FC<ManufacturingPlannerViewProps> = ({ manufacturing, inventory, manufacturingOrdersHook, purchaseOrdersHook, setCurrentView }) => {
    ...
    const manufacturableFamilias = useMemo(() => {
        return familias
            .filter(f => f.nodes?.some(n => n.data.type === 'productGenerator' || n.data.type === 'productGeneratorNode'))
    ...
    const handleAnalyze = () => {
        ...
        const result = analyzeManufacturingRun(order, components, undefined, selectedCodeFilter !== 'all' ? selectedCodeFilter : undefined);
                                    ...
                                    <div key={item.componentId} className="flex items-center gap-3 animate-fade-in">
                                        <div className="flex-grow p-4 bg-white rounded-2xl border-2 border-slate-100 shadow-sm flex items-center justify-between hover:border-indigo-200 transition-colors">
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-[9px] font-mono text-slate-400 font-bold mb-2">{comp?.sku}</p>
                                                <label className="flex items-center gap-1.5 cursor-pointer">
                                                    <input 
                                                        type="checkbox" 
                                                        checked={item.sourcingOverride === 'purchased'} 
                                                        onChange={e => {
                                                            const isPurchased = e.target.checked;
                                                            setOrder(prev => prev.map(i => i.componentId === item.componentId ? {...i, sourcingOverride: isPurchased ? 'purchased' : 'manufactured'} : i));
                                                            setAnalysisResult(null);
                                                        }}
                                                        className="w-3.5 h-3.5 rounded text-indigo-600 border-slate-300 focus:ring-indigo-500"
                                                    />
                                                    <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">
                                                        Comprar este lote pronto
                                                    </span>
                                                </label>
                                            </div>
                        ...
                        {!analysisResult ? (
                            ...
                            <div className="flex flex-col animate-fade-in">
                                ...
                                <div className="space-y-2 mb-6 -mr-2 pr-2">
                                    ...
                                    {analysisResult.requirements.map((req, ridx) => {
                                        const comp = inventory.findComponentById(req.id);
                                        const hasStock = (comp?.stock || 0) >= req.quantity;
                                        return (
                                            <div key={`${req.id}-${ridx}`} className={`flex justify-between items-center text-[11px] p-2 rounded-lg border ${hasStock ? 'bg-slate-50 border-slate-100' : 'bg-rose-50 border-rose-100'}`}>
                                                <div className="min-w-0 flex-grow">
                                                    <p className="font-bold text-slate-700">{req.name}{req.keyName ? ` (${req.keyName})` : ''}</p>
