...
export const ProductionOrdersView: React.FC<{
  ...
  customersHook: CustomersHook;
    ...
    const getTimestamp = () => new Date().toISOString().split('T')[0].replace(/-/g, '');
    ...
    const handleExportPDF = async () => {
        ...
        doc.setFontSize(18);
        doc.text("Relatório de Ordens de Montagem", 200, 22, { align: 'right' });
        doc.line(14, 28, 200, 28);

        autoTable(doc, {
            startY: 35,
            head: [['ID', 'Data', 'Status', 'Valor Total']],
            body: filteredOrders.map(o => [
                o.id.substring(0, 8),
                formatDateTime(o.createdAt),
                o.status.toUpperCase(),
                formatCurrency(o.saleDetails?.sellingPrice)
            ]),
            theme: 'grid',
            headStyles: { fillColor: [0, 43, 138] }
        });

        savePdfResiliently(doc, `ordens_montagem_${getTimestamp()}.pdf`);
    };
    ...
    if (productionOrdersHook.isLoading || inventory.isLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-64 space-y-4">
                <div className="w-12 h-12 border-4 border-slate-200 border-t-slate-900 rounded-full animate-spin"></div>
                <p className="text-slate-500 font-medium">Carregando ordens e inventário...</p>
            </div>
        );
    }
                                    ...
                                    <div className="p-4 cursor-pointer flex flex-col md:flex-row justify-between md:items-center gap-4 bg-white" onClick={() => setExpandedOrderId(isExpanded ? null : order.id)}>
                                        ...
                                        <div className="flex flex-grow justify-center md:justify-start px-4">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-1.5 h-8 ${customer ? 'bg-indigo-500' : 'bg-rose-500'} rounded-full`}></div>
                                                <div>
                                                    <span className="text-[9px] font-black text-slate-400 uppercase block">Cliente</span>
                                                    <span className="text-xs font-bold text-slate-700">{customer ? customer.name : (order.customerName || 'Não Identificado')}</span>
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="space-y-2">
                                                        {order.orderItems.map((item, idx) => {
                                                            const details = inventory.findKitById(item.id) || 
                                                                          inventory.findKitBySku(item.id) ||
                                                                          inventory.findComponentById(item.id) || 
                                                                          inventory.findComponentBySku(item.id) || 
                                                                          order.virtualComponents?.find(vc => vc.id === item.id || vc.sku === item.id);
                                                            
                                                            const displayName = details?.name || (inventory.isLoading ? 'Carregando...' : `Item não encontrado (${item.id || 'ID ausente'})`);
                                                            
                                                            return (
                                                                <div key={`${item.id}-${idx}`} className="flex justify-between items-center p-3 bg-white rounded-xl border border-slate-200 shadow-sm">
                                                                    <div className="flex items-center gap-3">
                                                                        ...
                                                                        {item.type === 'component' && <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-1.5 py-0.5 rounded">Item</span>}
                                                                    </div>
                                                                    <span className="text-[10px] font-mono text-slate-400">{details?.sku || item.id || '---'}</span>
                                                                </div>
                                                            );
                                                        })}
                                                    </div>
                                                ...
                                                <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        ...
                                                        <Button 
                                                            variant="secondary" 
                                                            size="sm" 
                                                            className="col-span-2 bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-100"
                                                            onClick={() => {
                                                                exportToBlingCSV([order], customersHook.customers, inventory);
                                                                addToast(`Ordem ${order.id} exportada para o Bling.`, "success");
                                                            }}
                                                        >
                                                            Exportar para Bling
                                                        </Button>
