...
export const FastenerCuttingView: React.FC<FastenerCuttingViewProps> = ({ inventory, cuttingOrdersHook }) => {
    ...
    const targetComponents = useMemo(() => {
        ...
        const targetDimsMap = new Map<string, ReturnType<typeof parseFastenerSku>>();
        ...
        return components.filter(c => {
            ...
            if (targetDims &&
                ...
                targetDimsMap.set(c.id, targetDims);
                return true;
            }
            return false;
        }).sort((a,b) => (targetDimsMap.get(b.id)?.comprimento || 0) - (targetDimsMap.get(a.id)?.comprimento || 0));
