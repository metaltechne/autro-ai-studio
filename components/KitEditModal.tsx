      ...
      <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
                <label className="block text-sm font-bold text-slate-800 uppercase tracking-tight mb-2">Processo de Fabricação Customizado</label>
                <select 
                ...
                >
                    <option value="">Padrão das Configurações Gerais</option>
                    {Object.entries(groupedFamilies).map(([tag, families]) => {
                        const fams = families as FamiliaComponente[];
                        return fams.length > 0 && (
                            <optgroup key={tag} label={tag === 'Generico' ? 'Outros Processos' : `Linha ${tag}`}>
                                {fams.map(f => (
                ...
                <p className="text-[10px] text-slate-500 font-bold uppercase mt-2 px-1">
                    {kitData.selectedFamiliaId 
                      ? "⚠️ Este kit usará um processo EXCLUSIVO para baixa de estoque e custos." 
                      : "✓ Este kit usará o processo mestre definido nas configurações gerais."}
                </p>
            </div>
        </div>
