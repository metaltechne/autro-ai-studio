...
export const RawMaterialsView: React.FC<RawMaterialsViewProps> = ({ inventory, onShowQRCode }) => {
    const { 
        components, addComponent, updateComponent, deleteComponent, addInventoryLog, getLogsForComponent,
    ...
    return (
        <div className="space-y-6">
            ...
            {isModalOpen && <MaterialModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingMaterial(null); }} onSave={async (comp, stockDelta) => {
                if (editingMaterial) {
                    await updateComponent(comp);
                    if (stockDelta && stockDelta !== 0) {
                        await addInventoryLog({ componentId: comp.id, type: stockDelta > 0 ? 'entrada' : 'saída', quantity: Math.abs(stockDelta), reason: stockDelta > 0 ? 'ajuste_inventario_positivo' : 'ajuste_inventario_negativo', notes: 'Ajuste manual no cadastro de matéria-prima.' });
                    }
                } else {
                    const added = await addComponent(comp);
                    if (added && stockDelta && stockDelta > 0) {
                        await addInventoryLog({ componentId: added.id, type: 'entrada', quantity: stockDelta, reason: 'estoque_inicial', notes: 'Estoque inicial da matéria-prima.' });
                    }
                }
            ...
            >
                <p>Deseja descartar seu rascunho local e recarregar da nuvem? Esta ação não pode ser desfeita.</p>
            </ConfirmationModal>
        </div>
    );
};
...
const MaterialModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSave: (material: any, stockDelta?: number) => Promise<any>;
    materialToEdit: Component | null;
}> = ({ isOpen, onClose, onSave, materialToEdit }) => {
    const [data, setData] = useState<Partial<Component>>(materialToEdit || { 
        name: '', 
        sku: '', 
        purchaseCost: 0, 
        unitCategory: 'count', 
    ...
    const [newStock, setNewStock] = useState<number>(materialToEdit?.stock || 0);

    const handleSave = async () => {
        if (!data.name || !data.sku) {
            return;
        }
        const stockDelta = newStock - (materialToEdit?.stock || 0);
        await onSave(data, stockDelta);
        onClose();
    };

    const handleCategoryChange = (val: string) => {
        let category: 'weight'|'length'|'count' = 'count';
        let unit = 'un';
        if (val === 'weight') {
            category = 'weight';
            unit = 'kg';
        ...
        setData({...data, unitCategory: category, purchaseUnit: unit, consumptionUnit: unit});
    };

    const getUnitLabel = () => {
        if (data.unitCategory === 'length') return 'Metros (M)';
        if (data.unitCategory === 'weight') return 'Quilos (Kg)';
        return 'Unidades (Un)';
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={materialToEdit ? "Editar Material" : "Cadastrar Matéria-Prima"}>
            <div className="space-y-4">
                ...
                <Input label={`${materialToEdit ? "Estoque Atual" : "Estoque Inicial"} (em ${getUnitLabel()})`} type="number" min="0" value={newStock} onChange={e => setNewStock(parseFloat(e.target.value) || 0)} helpText={`Informe a quantidade em ${getUnitLabel()}. Ex: Para 2 barras de 6m, digite 12.`} />
                <div className="grid grid-cols-2 gap-4">
                    <Select label="Categoria" value={data.unitCategory} onChange={e => handleCategoryChange(e.target.value)}>
                        <option value="weight">Peso (Kg)</option>
                        <option value="length">Comprimento (M)</option>
                        <option value="count">Contagem (Un)</option>
                    </Select>
                    <Input label="Custo da Compra (R$)" type="number" value={data.purchaseCost} onChange={e => setData({...data, purchaseCost: parseFloat(e.target.value)})} helpText="Preço pago por barra/caixa/pacote." />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <Input label={`Qtd que vem na Compra (em ${getUnitLabel()})`} type="number" value={data.purchaseQuantity || 1} onChange={e => setData({...data, purchaseQuantity: parseFloat(e.target.value)})} helpText={`Quantos ${getUnitLabel()} vêm ao comprar? Ex: 6 (se for barra de 6m)`} />
                    <div className="flex items-end pb-1">
                        <p className="text-[10px] text-slate-500 italic pb-2">O custo unitário (por {getUnitLabel()}) será calculado automaticamente e usado para abater a produção.</p>
                    </div>
                </div>
