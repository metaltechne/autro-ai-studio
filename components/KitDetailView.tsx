...
export const KitDetailView: React.FC<KitDetailViewProps> = ({ kitId, inventory, manufacturing, setCurrentView, onShowQRCode }) => {
    ...
    const costDetails = useMemo(() => {
        ...
        // Helper to aggregate breakdown items
        const aggregatedBreakdown = new Map<string, KitCostBreakdownItem>();
        
        const addToBreakdown = (item: KitCostBreakdownItem) => {
            const existing = aggregatedBreakdown.get(item.sku);
            if (existing) {
                existing.quantity += item.quantity;
                existing.totalCost += item.totalCost;
                // Keep the existing costBreakdown or merge if needed, 
                // but usually they are identical for the same SKU
            } else {
                aggregatedBreakdown.set(item.sku, { ...item });
            }
        };
        ...
        if (kit.requiredFasteners && Array.isArray(kit.requiredFasteners)) {
            kit.requiredFasteners.forEach(rf => {
                ...
                const fixSFamilia = manufacturing.familias.find(f => f.id === 'fam-MONTAGEM-FIX-S') || 
                                   manufacturing.familias.find(f => f.nome === 'Montagem FIX-S (Interna)') ||
                                   manufacturing.familias.find(f => f.nome?.toLowerCase() === 'montagem fix-s' || f.nome?.toLowerCase() === 'fix-s');
                const porPFamilia = manufacturing.familias.find(f => f.id === 'fam-MONTAGEM-POR-P') ||
                                   manufacturing.familias.find(f => f.nome === 'Montagem POR-P') ||
                                   manufacturing.familias.find(f => f.nome?.toLowerCase() === 'montagem por-p' || f.nome?.toLowerCase() === 'por-p');
                
                let familiaToUse = isNut ? porPFamilia : fixSFamilia;
                
                if (isNut && draftKit?.selectedNutFamiliaId) {
                    familiaToUse = familiaMap.get(draftKit.selectedNutFamiliaId) || familiaToUse;
                } else if (!isNut && draftKit?.selectedFamiliaId) {
                    familiaToUse = familiaMap.get(draftKit.selectedFamiliaId) || familiaToUse;
                }
                ...
                addToBreakdown({
                    name, 
                    sku: `${familyTag}-${rf.dimension.toUpperCase().replace('MM', '')}`, 
                    quantity: rf.quantity,
                    unitCost: unitCost, 
                    totalCost: itemTotalCost, 
                    type: isNut ? 'Fixador' : 'Fixador',
        ...
        // 3. Processar Copos Automáticos (Igual ao KitsView)
            ...
            kit.requiredFasteners.forEach(rf => {
                ...
                if (!isNut) {
                    ...
                    if (!isNaN(bitola)) {
                        ...
                        if (copoFamilia) {
                            const res = evaluateProcess(copoFamilia, { bitola: copoBitola }, components, {}, { allFamilias: manufacturing.familias });
        ...
        const defaultBreakdown = Array.from(aggregatedBreakdown.values());
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Família para Parafusos/Hastes</label>
                                        <Select 
                                        ...
                                        >
                                            ...
                                            {Object.entries(availableFamiliasGrouped).map(([tag, families]) => {
                                                const fams = families as FamiliaComponente[];
                                                return fams.length > 0 && (
                                                    <optgroup key={`parafuso-${tag}`} label={tag === 'Generico' ? 'Processos Genéricos' : `Linha ${tag}`}>
                                                        {fams.map(f => (
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Família para Porcas</label>
                                        <Select 
                                            value={draftKit.selectedNutFamiliaId || ''} 
                                            onChange={e => setDraftKit(prev => prev ? { ...prev, selectedNutFamiliaId: e.target.value } : null)}
                                            className="font-bold text-slate-900 w-full"
                                        >
                                            ...
                                            {Object.entries(availableFamiliasGrouped).map(([tag, families]) => {
                                                const fams = families as FamiliaComponente[];
                                                return fams.length > 0 && (
                                                    <optgroup key={`porca-${tag}`} label={tag === 'Generico' ? 'Processos Genéricos' : `Linha ${tag}`}>
                                                        {fams.map(f => (
                                                            <option key={f.id} value={f.id}>{f.nome}</option>
                                                        ))}
                                                    </optgroup>
                                ...
                                <p className="mt-4 text-[11px] text-slate-500 font-medium leading-tight">
                                    {draftKit.selectedFamiliaId || draftKit.selectedNutFamiliaId
                                      ? "⚠️ Este kit possui famílias de custos customizadas. A baixa de estoque seguirá estas regras." 
                                      : "✓ Utilizando detecção automática e famílias padrões. Você pode forçar matrizes específicas de custos escolhendo acima."}
                                </p>
                            </div>
                        </div>
                    </Card>
                                                        ...
                                                        <td colSpan={4} className="px-4 py-3">
                                                            <div className="pl-8">
                                                                ...
                                                                <div className="space-y-1">
                                                                    {item.costBreakdown!.map((step) => (
                                                                        <div key={step.nodeId || step.name} className="flex justify-between items-center text-sm border-b border-slate-200 last:border-0 pb-1 last:pb-0">
