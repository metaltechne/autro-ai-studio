    ...
    const costDetails = useMemo(() => {
        ...
        if (kit.requiredFasteners && Array.isArray(kit.requiredFasteners)) {
            kit.requiredFasteners.forEach(rf => {
                ...
                const fixSFamilia = manufacturing.familias.find(f => f.id === 'fam-MONTAGEM-FIX-S' || f.nome?.toLowerCase() === 'montagem fix-s' || f.nome?.toLowerCase() === 'fix-s' || f.nome?.toLowerCase().includes('fix'));
                const porPFamilia = manufacturing.familias.find(f => f.id === 'fam-MONTAGEM-POR-P' || f.nome?.toLowerCase() === 'montagem por-p' || f.nome?.toLowerCase() === 'por-p' || f.nome?.toLowerCase().includes('por'));
