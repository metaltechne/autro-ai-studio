                    ...
                    <Card className="border-none shadow-soft">
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Processo de Fabricação do Kit</label>
                                <Select 
                                    value={draftKit.selectedFamiliaId || ''} 
                                    onChange={e => setDraftKit(prev => prev ? { ...prev, selectedFamiliaId: e.target.value } : null)}
                                    className="font-bold text-slate-900"
                                >
                                    <option value="">Padrão: {generatorFamilias.find(f => f.id === financialSettings?.preferredFastenerFamiliaId)?.nome || '(Não definido)'}</option>
                                    {Object.entries(availableFamiliasGrouped).map(([tag, families]) => {
                                        const fams = families as FamiliaComponente[];
                                        return fams.length > 0 && (
                                            <optgroup key={tag} label={tag === 'Generico' ? 'Processos Genéricos' : `Linha ${tag}`}>
                                                {fams.map(f => (
                                ...
                                <p className="mt-3 text-[11px] text-slate-500 font-medium leading-tight">
                                    {draftKit.selectedFamiliaId 
                                      ? "⚠️ Este kit possui um processo customizado. A baixa de estoque seguirá as regras exclusivas deste fluxo." 
                                      : "✓ Utilizando a configuração global. Alterar esta opção afetará apenas este Kit específico."}
                                </p>
                            </div>
                        </div>
                    </Card>
