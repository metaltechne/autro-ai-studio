    ...
    return (
        <div className="h-[calc(100vh-8rem)] flex flex-col">
            <div className="flex justify-between items-center mb-4 flex-shrink-0">
                ...
                <div className="flex gap-2">
                    {cuttingOrders.length > 0 && (
                        <Button onClick={() => setIsConfirmingClearAll(true)} variant="danger" className="bg-red-600 hover:bg-red-700">
                            Limpar Tudo
                        </Button>
                    )}
            ...
            <ConfirmationModal
                ...
                onConfirm={async () => {
                    await cuttingOrdersHook.clearAllCuttingOrders();
                    setIsConfirmingClearAll(false);
                }}
                title="Limpar Todas as Ordens de Corte"
                confirmText="Sim, Deletar Tudo"
                variant="danger"
            >
                <div className="space-y-3">
                    <p className="text-sm font-bold text-red-600">ATENÇÃO: Esta ação é irreversível!</p>
                    <p className="text-sm text-gray-600">Isso apagará TODAS as {cuttingOrders.length} ordens de corte cadastradas no sistema. Peças em estoque não serão afetadas.</p>
                </div>
            </ConfirmationModal>
