...
export const ComponentEditModal: React.FC<ComponentEditModalProps> = ({ isOpen, onClose, onSave, componentToEdit, familias }) => {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        const stockDelta = newStock - (componentToEdit?.stock || 0);
        await onSave(data as Component, stockDelta);
        setIsSaving(false);
        onClose();
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Editar Componente: ${componentToEdit.name}`}>
            <div className="space-y-4">
                ...
                <Input label="Quantidade em Estoque Atual" type="number" name="stock" value={newStock} onChange={e => setNewStock(parseFloat(e.target.value) || 0)} />
