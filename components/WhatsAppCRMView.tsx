...
import { Search, Plus, MessageCircle, Phone, Building, Calendar, DollarSign, ExternalLink, Save, ArrowLeft, Copy, Check, Tag, CheckCircle2, XCircle, Megaphone, Clock, Bell, X } from 'lucide-react';
...
export const WhatsAppCRMView: React.FC<WhatsAppCRMViewProps> = ({ inventory, productionOrdersHook }) => {
    const { leads, deals, addLead, addDeal, updateLead, addDealActivity, deleteLead } = useSalesFunnel();
                    ...
                    <div className="divide-y divide-slate-100">
                        {filteredLeads.map(lead => (
                            <div
                                ...
                                className={`w-full text-left p-4 hover:bg-slate-50 transition-colors cursor-pointer flex justify-between items-start ${selectedLeadId === lead.id ? 'bg-blue-50/50 border-l-4 border-autro-primary' : 'border-l-4 border-transparent'}`}
                            >
