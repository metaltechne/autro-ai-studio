  ...
  <li className="relative">
    ...
    >
      <span className={`flex-shrink-0 transition-transform duration-300 ${isActive ? 'text-autro-primary scale-105' : 'text-slate-500 group-hover:text-slate-300'}`}>
        <div className="h-3 w-3 md:h-4 md:w-4 flex items-center justify-center">
            {icon}
        </div>
      </span>
    ...
    >
        ...
        <div className="p-4 border-t border-white/5 bg-black/20">
            <div className="flex items-center justify-center mb-2">
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-full bg-autro-primary/20 text-autro-primary border border-autro-primary/30 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                    V1.0.0 PRODUCTION
                </span>
            </div>
            <button
