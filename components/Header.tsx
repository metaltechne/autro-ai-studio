...
const viewTitles: Record<View, string> = {
    ...
    [View.QUICK_INVENTORY]: 'Entrada Rápida (Insumos)',
    ...
    [View.RAW_MATERIALS]: 'Matéria-Prima',
...
export const Header: React.FC<HeaderProps> = ({ currentView, onSave, isDirty, savingStatus }) => {
    ...
    <header className="flex items-center justify-between h-20 px-8 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-10 print-hide">
      ...
      <div className="flex items-center space-x-8">
        {onSave && (
          <button
            onClick={onSave}
            disabled={savingStatus === 'saving' || !isDirty}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300 ${
              isDirty 
                ? 'text-white bg-autro-primary hover:bg-autro-primary/90' 
                : 'text-slate-400 bg-slate-100 cursor-not-allowed'
            }`}
          >
            {savingStatus === 'saving' ? 'Salvando...' : isDirty ? 'Salvar Alterações' : 'Tudo Salvo'}
          </button>
        )}
