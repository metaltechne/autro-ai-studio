                                                    ...
                                                    <td className={`px-4 py-2 text-right font-bold ${item.netToBuy > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                                        {item.netToBuy > 0 ? (
                                                            <>
                                                                ...
                                                                {item.toBuyPurchaseUnits !== undefined && item.toBuyPurchaseUnits > 0 && (
                                                                    <span className="text-xs text-gray-500 block font-normal">
                                                                        ({formatNumber(item.toBuyPurchaseUnits, 1)} {item.purchaseUnit})
                                                                    </span>
                                                                )}
                                                            </>
                                                        ) : (
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                           {purchasePlan.map(rec => (
                                                <tr key={rec.componentId} className={`hover:bg-gray-50 transition-colors ${selectedPurchases.has(rec.componentId) ? 'bg-blue-50/50' : ''}`}>
                                                   ...
                                                   <td className="px-4 py-3 text-right text-gray-600">{Math.floor(rec.inStock)}</td>
                                                   <td className="px-4 py-3 text-right font-bold text-blue-700 bg-blue-50 rounded-lg">
                                                       {Math.ceil(rec.toOrder)} {rec.consumptionUnit}
                                                       {rec.toOrderPurchaseUnits !== undefined && rec.toOrderPurchaseUnits > 0 && rec.purchaseUnit !== rec.consumptionUnit && (
                                                           <span className="text-[10px] text-gray-500 block font-normal">
                                                               ({formatNumber(rec.toOrderPurchaseUnits, 1)} {rec.purchaseUnit})
                                                           </span>
                                                       )}
                                                   </td>
                                               </tr>
