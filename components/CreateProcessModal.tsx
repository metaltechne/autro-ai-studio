...
const TEMPLATES = [
    {
        id: 'duplicate',
        title: 'Duplicar Processo Existente',
        subtitle: 'A partir da biblioteca',
        description: 'Copie um processo que já existe e modifique seus nós.',
        category: 'both' as ProcessCategory,
        icon: <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>,
        nodes: [],
        edges: []
    },
    {
...
export const CreateProcessModal: React.FC<CreateProcessModalProps> = ({ isOpen, onClose, onCreate, existingFamilies }) => {
    const [name, setName] = useState('');
    const [selectedId, setSelectedId] = useState('duplicate');
    const [selectedSourceFamilyId, setSelectedSourceFamilyId] = useState(existingFamilies[0]?.id || '');

    const handleCreate = () => {
        if (!name.trim()) return;

        if (selectedId === 'duplicate') {
            const sourceFamily = existingFamilies.find(f => f.id === selectedSourceFamilyId);
            if (!sourceFamily) return;
            
            // Generate distinct IDs mapping to avoid ID collision
            const nodeMap = new Map();
            const duplicatedNodes = sourceFamily.nodes.map((n: any) => {
                const newId = `node_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
                nodeMap.set(n.id, newId);
                return { ...n, id: newId };
            });
            const duplicatedEdges = sourceFamily.edges.map((e: any) => ({
                ...e,
            ...
            onCreate(
                name.trim(),
                sourceFamily.tipo || 'generator',
                sourceFamily.categoria || 'manufacturing',
                { nodes: duplicatedNodes, edges: duplicatedEdges }
            );
        } else {
                ...
                <div className="bg-slate-900 p-8 rounded-3xl text-white shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative">
                        <Input 
                            ...
                            placeholder="NOME DO NOVO PROCESSO (Ex: FIX-P SVD)"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {TEMPLATES.map(template => (
                        <div
                            ...
                            className={`p-4 rounded-2xl border-2 transition-all cursor-pointer group flex flex-col ${
                        ...
                        >
                            ...
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-2">{template.subtitle}</p>
                            <p className="text-[9px] leading-relaxed text-slate-500 font-medium flex-grow">{template.description}</p>
                        </div>
                    ))}
                </div>

                {selectedId === 'duplicate' && existingFamilies.length > 0 && (
                    <div className="p-5 bg-slate-50 border-2 border-slate-200 rounded-2xl">
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Qual processo base você deseja copiar?</label>
                        <select 
                            value={selectedSourceFamilyId}
                            onChange={(e) => setSelectedSourceFamilyId(e.target.value)}
                            className="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-3 font-black text-slate-800 uppercase tracking-tight focus:border-autro-primary focus:ring-4 focus:ring-autro-primary/10 outline-none"
                        >
                            <option value="" disabled>SELECIONE UM PROCESSO DA BIBLIOTECA</option>
