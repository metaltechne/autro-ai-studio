...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(({ label, id, className = '', helpText, ...props }, ref) => {
  return (
    <div className="w-full">
      ...
      <input
        ...
        {...props}
      />
      {helpText && <p className="mt-1.5 ml-1 text-[11px] text-slate-500 leading-tight">{helpText}</p>}
    </div>
  );
});
