...
export const SpeedDial: React.FC<SpeedDialProps> = ({ actions, isMobile = false }) => {
  ...
  const getActionStyle = (index: number): React.CSSProperties => {
    if (!isOpen) {
      return { transform: 'translateY(20px) scale(0.8)', opacity: 0, pointerEvents: 'none' };
    }
    return {
      transform: 'translateY(0) scale(1)',
      opacity: 1,
      transitionDelay: `${(actions.length - 1 - index) * 0.05}s`,
      pointerEvents: 'auto',
    };
  };
  ...
  return (
    <div ref={wrapperRef} className={`fixed ${isMobile ? 'bottom-20' : 'bottom-6'} right-6 z-[45] print-hide`}>
      <div className="relative flex flex-col items-center">
        {/* Action buttons */}
        <div 
            className={`absolute bottom-full mb-4 flex flex-col items-center gap-4 transition-all duration-300 ${isOpen ? 'visible' : 'invisible'}`}
        >
