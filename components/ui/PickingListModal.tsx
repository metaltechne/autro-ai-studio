...
export const PickingListModal: React.FC<PickingListModalProps> = ({ isOpen, onClose, order, inventory }) => {
    ...
    const productsToMake = (isProductionOrder ? order.orderItems : order.orderItems).map(item => {
        if (isProductionOrder) {
            const product = inventory.findKitById((item as ProductionOrderItem).kitId);
            return {
                quantity: item.quantity,
                name: product?.name || 'Item não encontrado',
                sku: product?.sku || 'N/A'
            };
        } else { // ManufacturingOrder
            const componentId = (item as ManufacturingOrderItem).componentId;
            const product = inventory.findComponentById(componentId);
