...
export const KitsByBrandView: React.FC<KitsByBrandViewProps> = ({ inventory, manufacturing }) => {
  ...
  const kitCostsAndSales = useMemo(() => {
    ...
    const componentSkuMap = new Map<string, Component>(components.filter(c => !!c.sku).map(c => [c.sku.toUpperCase(), c]));
      ...
      const getFastenerDetails = (dimension: string, familia: any): { unitCost: number, breakdown: CostStep[] } => {
        if (!familia) return { unitCost: 0, breakdown: [] };
        ...
        (kit.components || []).forEach((kc: KitComponent) => {
          ...
          if (component) {
            ...
            if (component.type === 'raw_material' || component.sourcing === 'purchased') {
              ...
              // Try to get breakdown if it's manufactured
              if (component.familiaId) {
                  const familia = familias.find(f => f.id === component.familiaId);
                  if (familia) {
                      // Attempt to evaluate with empty vars to see if we get a breakdown
                      // Standard components might have fixed costs or need specific vars, 
                      // but we try to see what we can get.
                      const res = evaluateProcess(familia, {}, inventory.components, {}, { allFamilias: familias, workStations: manufacturing.workStations, consumables: manufacturing.consumables, operations: manufacturing.standardOperations });
      ...
      if (fixSFamilia && hasScrews) {
        ...
        kit.requiredFasteners?.forEach(rf => {
          const details = getFastenerDetails(rf.dimension, fixSFamilia);
          const unitCost = details.unitCost;
          const itemTotalCost = unitCost * rf.quantity;
          ...
          fixSBreakdown.push({
            ...
            unitCost, totalCost: itemTotalCost, type: 'Fixador',
            costBreakdown: details.breakdown
          });
        });
        options.fixS = {
      ...
      if (fixPFamilia && hasScrews) {
        ...
        kit.requiredFasteners?.forEach(rf => {
          const details = getFastenerDetails(rf.dimension, fixPFamilia);
          const unitCost = details.unitCost;
          const itemTotalCost = unitCost * rf.quantity;
  ...
  const handlePrintTechnicalSheet = async (kit: Kit, detailsOverride?: KitCostDetails) => {
    ...
    details.breakdown.forEach(item => {
        const component = components.find(c => c.sku === item.sku);
        const fabCost = component ? (component.custoFabricacao || 0) : 0;
        const matCost = component ? (component.custoMateriaPrima || 0) : item.unitCost;
        ...
        if (item.costBreakdown && item.costBreakdown.length > 0) {
            item.costBreakdown.forEach(step => {
                ...
                tableBody.push([
                    '',
                    `   > ${step.name} (${step.type === 'labor' ? 'Mão de Obra' : 'Insumo'})`,
                    '',
                    timeStr,
                    step.details || '',
