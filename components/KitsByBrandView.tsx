...
export const KitsByBrandView: React.FC<KitsByBrandViewProps> = ({ inventory, manufacturing }) => {
    ...
    const getFastenerDetails = (dimension: string, familia: any): { unitCost: number, materialCost: number, fabricationCost: number, breakdown: CostStep[] } => {
      ...
      if (!isNaN(bitola)) {
          const result = evaluateProcess(
              ...
              components,
              {},
              { consumables: manufacturing.consumables, workStations: manufacturing.workStations, operations: manufacturing.standardOperations }
          );
          return {
      ...
      try {
        ...
        if (kit.components) {
          kit.components.forEach(kc => {
            ...
            if (component) {
              const unitCost = (component.custoMateriaPrima || 0) + (component.custoFabricacao || 0);
              const itemTotalCost = unitCost * kc.quantity;
              
              const itemBreakdown: CostStep[] = [];
              if (component.familiaId) {
                  const familia = manufacturing.familias.find(f => f.id === component.familiaId);
                  if (familia) {
                      const variables = component.dimensions || {};
                      const result = evaluateProcess(
                          familia, 
                          variables, 
                          components, 
                      ...
                      itemBreakdown.push(...result.costBreakdown);
                  }
              }
        ...
        console.error(`Error calculating details Map:`, err);
      }
    }
    return detailsMap;
  }, [kits, familias, components, calculateSaleDetails, manufacturing.consumables, manufacturing.workStations, manufacturing.standardOperations]);
  ...
  const handlePrintTechnicalSheet = async (kit: Kit, detailsOverride?: KitCostDetails) => {
    ...
    doc.text(`${kit.name}${labelSuffix}`, 14, 42);
    doc.setFontSize(10);
