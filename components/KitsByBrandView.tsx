    ...
    for (const kit of (kits || [])) {
      try {
        ...
        if (kit.selectedFamiliaId) {
            defaultFastenerName = kit.selectedFamiliaId === 'fixS' ? 'FIX-S' : (kit.selectedFamiliaId === 'fixP' ? 'FIX-P' : (manufacturing.familias.find(f => f.id === kit.selectedFamiliaId)?.nome || kit.selectedFamiliaId));
        }
        if (kit.selectedNutFamiliaId) {
            defaultNutName = kit.selectedNutFamiliaId === 'porP' ? 'POR-P' : (manufacturing.familias.find(f => f.id === kit.selectedNutFamiliaId)?.nome || kit.selectedNutFamiliaId);
        }
        ...
        if (fixSFamilia) {
          options.fixS = {
            ...
            selectedNutFamiliaName: defaultNutName || 'POR-P'
          };
        }
        ...
        detailsMap.set(kit.id, {
          ...
          saleDetails,
          label: 'Padrão',
          selectedFastenerFamiliaName: defaultFastenerName || 'FIX-S',
          selectedNutFamiliaName: defaultNutName || 'POR-P',
          options
        });
      } catch (err) {
