...
export const ProductDesignerView: React.FC<ProductDesignerViewProps> = ({ manufacturing, inventory, setCurrentView }) => {
    ...
    const handleExportAllExcel = () => {
        ...
        familias.forEach(f => {
            const operations = f.nodes.filter(n => n.data.type === 'etapaFabricacao');
            const inputs = f.nodes.filter(n => 
                n.data.type === 'materiaPrima' || 
                n.data.type === 'inventoryComponent' || 
                n.data.type === 'materialMapping'
            );
            const dimensions = f.nodes.find(n => n.data.type === 'dimensionTable' || n.data.type === 'dnaTable');

            if (operations.length === 0 && inputs.length === 0) {
                // If no detail, just add the family basic info
                 data.push({
                    'Família': f.nome,
                    'Categoria': f.category,
                    'Detalhamento': 'Apenas Cadastro Base'
                });
            } else {
                // Add rows for operations
                operations.forEach((op, idx) => {
                    data.push({
                        ...
                        'Sequência': idx + 1,
                        'Nome': op.data.label,
                        'Duração (s)': op.data.manualTimeSeconds || 0,
                        'Posto': op.data.manualOperatorId || '',
                        'Custo Est. (R$)': op.data.cost || 0
                    });
                });
                // Add rows for inputs
                inputs.forEach((inp, idx) => {
                    data.push({
                        'Família': f.nome,
                        'Categoria': f.category,
                        'Tipo': 'Ingrediente/Matéria-Prima',
                        'Sequência': idx + 1,
                        'Nome': inp.data.label,
                        'Template SKU': inp.data.componentIdTemplate || inp.data.sourceSku || '',
                        'Consumo': inp.data.consumption || 1
                    });
                });
            }
        });
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Produtos e Processos");
        ...
        saveAs(finalData, `export_produtos_processos_${new Date().toISOString().split('T')[0]}.xlsx`);
        addToast("Exportação de produtos concluída!", "success");
    };
            ...
            <div className="w-80 bg-white border-r flex flex-col flex-shrink-0">
                <div className="p-4 border-b">
                    ...
                    <div className="flex flex-col gap-2 mb-4">
                        <Button onClick={() => setIsCreating(true)} className="w-full">Novo Produto / Kit</Button>
                        <Button onClick={handleExportAllExcel} variant="secondary" className="w-full flex items-center justify-center gap-2 text-xs py-1.5 h-auto uppercase font-bold">
                            <Download size={14} />
                            Exportar Tudo (Excel)
                        </Button>
                    </div>
