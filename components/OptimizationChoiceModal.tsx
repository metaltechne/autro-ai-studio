...
}> = ({ shortage, onCut, onManufacture, onBuy, isProcessing, resolution, canViewCosts }) => {
                                ...
                                {shortage.substitutionOptions.map((sub, index) => (
                                    <div key={index} className="p-2 bg-blue-50 border border-blue-200 rounded-md flex justify-between items-center">
                                        <div className="text-xs text-blue-900">
                                            ...
                                            <p>Estoque: {sub.sourceComponent.stock} | {canViewCosts && `Custo Extra: ${formatCurrency(sub.costOfCutting)}/un | `}Parado há: {sub.sourceComponentAgeDays} dias</p>
                                        </div>
...
export const OptimizationChoiceModal: React.FC<OptimizationChoiceModalProps> = ({
    ...
    const handleBuy = async (shortage: ProductionScenarioShortage) => {
        ...
        try {
            ...
            if (comp) {
                sku = comp.sku;
                // Se tem família, prioriza manufactured mesmo que o sourcing esteja errado
                sourcing = (comp.familiaId || comp.sourcing === 'manufactured') ? 'manufactured' : 'purchased';
            } else if (targetComponentId.startsWith('unknown-')) {
                sku = targetComponentId.replace('unknown-', '');
                // Heurística: se o SKU começa com FIX-S ou tem padrão de fabricado, default para manufactured
                const isLikelyManufactured = sku.startsWith('FIX-S') || sku.startsWith('FIX-P') || sku.includes('USINADO');
                const defaultSourcing = isLikelyManufactured ? 'manufactured' : 'purchased';
