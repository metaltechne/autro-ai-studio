...
const TableInput = ({ value, onSave, type = "number", step, placeholder, className = "" }: { value: any, onSave: (val: any) => void, type?: string, step?: string, placeholder?: string, className?: string }) => {
    ...
    return (
        <input type={type} step={step} value={localValue} placeholder={placeholder} onChange={(e) => setLocalValue(e.target.value)} onBlur={handleBlur} onKeyDown={handleKeyDown}
            className={`w-full border-none text-center bg-slate-100 focus:ring-2 focus:ring-blue-500 rounded font-black h-8 text-slate-800 placeholder:font-normal placeholder:text-slate-400 text-xs ${className}`}
        />
    );
};
...
const NodeContainer = ({ children, selected, title, icon, colorClass, className = '', onDuplicate, onDelete, cost, isTotal }: any) => {
    return (
        <div className={`rounded-2xl bg-white shadow-xl border-2 transition-all duration-300 min-w-[320px] overflow-hidden ${selected ? 'border-blue-500 ring-8 ring-blue-500/10' : colorClass} ${className}`}>
            <div className={`px-4 py-3 border-b flex items-center gap-3 ${selected ? 'bg-blue-50' : 'bg-slate-50'} cursor-grab active:cursor-grabbing`}>
                 <div className="flex-shrink-0">{icon}</div>
                 <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-900 flex-grow">{title}</span>
                 <NodeActions onDuplicate={onDuplicate} onDelete={onDelete} />
            </div>
            {cost !== undefined && (
                <div className={`px-4 py-2 flex justify-between items-center border-b ${isTotal ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600'}`}>
                    <span className="text-[9px] font-black uppercase tracking-widest opacity-60">{isTotal ? 'CUSTO TOTAL' : 'CUSTO'}</span>
                    <span className={`font-black ${isTotal ? 'text-emerald-400 text-base' : 'text-slate-900 text-xs'}`}>{formatCurrency(cost)}</span>
                </div>
            )}
    ...
    <div className="relative">
        <NodeContainer selected={selected} title="DNA GEOMÉTRICO (TABELA)" colorClass="border-blue-200" onDuplicate={data.duplicateNode} onDelete={data.deleteNode}>
            <div className="nodrag nowheel space-y-4">
                ...
                <div className="max-h-48 overflow-y-auto border rounded-lg bg-white shadow-inner scrollbar-thin">
        ...
        <div className="relative">
            <NodeContainer selected={selected} title="MAPEAMENTO DE INSUMOS" colorClass="border-emerald-200" className="w-[500px]" onDuplicate={data.duplicateNode} onDelete={data.deleteNode} cost={data.cost}>
                <div className="space-y-4">
                    ...
                    <div className="overflow-hidden rounded-xl border border-emerald-100 bg-white nodrag nowheel max-h-64 overflow-y-auto scrollbar-thin">
                        <table className="w-full text-[10px]">
                            <thead className="bg-emerald-600 text-white font-black uppercase sticky top-0">
                                <tr><th className="p-2 text-left">MEDIDA</th><th className="p-2 text-left">INSUMO</th><th className="p-2 w-16 text-center">CONS</th><th className="p-2 w-12 text-center">RAMIF</th></tr>
                            </thead>
                                    ...
                                    <tr key={dim.id} className="h-10 relative">
                                        ...
                                        <td className="p-1">
                                            <select value={dim.baseMaterialId || ''} onChange={e => data.updateDimension?.(dim.id, { baseMaterialId: e.target.value })} 
                                                className="w-full text-[10px] border-none bg-slate-50 focus:ring-0 font-black uppercase text-slate-600 rounded">
        ...
        <div className="relative">
            <NodeContainer selected={selected} title="SERVIÇO POR MEDIDA" colorClass="border-indigo-200" onDuplicate={data.duplicateNode} onDelete={data.deleteNode} cost={data.cost}>
                <div className="space-y-4">
                    ...
                    <div className="overflow-hidden rounded-xl border border-indigo-100 bg-white nodrag nowheel max-h-64 overflow-y-auto scrollbar-thin">
                        <table className="w-full text-[10px]">
                            <thead className="bg-indigo-600 text-white font-black uppercase sticky top-0">
                                <tr><th className="p-2 text-left">MEDIDA</th><th className="p-2 text-right">VALOR (R$)</th><th className="p-2 w-12 text-center">RAMIF</th></tr>
                            </thead>
...
export const FabricationNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => {
    ...
    const currOpId = data.manualOperatorId || (op ? op.workStationId : '');
    const currTime = data.manualTimeSeconds !== undefined ? data.manualTimeSeconds : (op?.timeSeconds || 0);
    return (
        <div className="relative">
                ...
                <div className="space-y-4 nodrag">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <Select label="OPERADOR" value={currOpId} onChange={e => data.updateNodeOperationDetails?.({ manualOperatorId: e.target.value })}><option value="">PADRÃO</option>{data.operators?.map(ws => <option key={ws.id} value={ws.id}>{ws.name}</option>)}</Select>
                            <Input label="TEMPO (s)" type="number" value={currTime} onChange={e => data.updateNodeOperationDetails?.({ manualTimeSeconds: parseFloat(e.target.value) })} />
                        </div>
                    </div>
                </div>
            </NodeContainer>
...
export const InventoryNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => (
    <div className="relative"><Handle type="target" position={Position.Left} style={handleStyle} /><NodeContainer selected={selected} title="ITEM DE ESTOQUE" colorClass="border-purple-200" cost={data.cost} onDuplicate={data.duplicateNode} onDelete={data.deleteNode}><div className="space-y-4 nodrag">{data.componentIdTemplate ? <div className="p-3 rounded-xl bg-blue-50 border text-[9px] font-black uppercase text-blue-700 tracking-widest">🧬 Herança DNA Ativa</div> : <Select value={data.componentId || ''} onChange={e => data.updateNodeComponentDetails?.({ componentId: e.target.value, componentIdTemplate: '' }, [])}><option value="">SELECIONAR...</option>{(data.getInventoryComponentOptions?.() || []).map((c: any) => <option key={c.id} value={c.id}>{c.sku}</option>)}</Select>}</div></NodeContainer><Handle type="source" position={Position.Right} style={handleStyle} /></div>
));
    ...
    return (
        <div className="relative">
            <NodeContainer selected={selected} title="PROCESSO POR MEDIDA" colorClass="border-purple-200" onDuplicate={data.duplicateNode} onDelete={data.deleteNode} cost={data.cost}>
                <div className="space-y-4">
                    <Handle type="target" position={Position.Left} id="data-in" style={dataHandleStyle} />
                    <div className="overflow-hidden rounded-xl border border-purple-100 bg-white nodrag nowheel max-h-64 overflow-y-auto scrollbar-thin">
                        <table className="w-full text-[10px]">
...
export const DimensionTableUI: React.FC<{ dimensions: ProcessDimension[] | undefined; onUpdate: (id: string, data: Partial<ProcessDimension>) => void; onAdd: () => void; onDelete: (id: string) => void; }> = ({ dimensions, onUpdate, onAdd, onDelete }) => (
    <div className="space-y-4">
        ...
        <div className="max-h-64 overflow-y-auto border rounded-xl bg-white shadow-inner">
