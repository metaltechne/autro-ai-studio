...
const TableInput = ({ value, onSave, type = "number", step, placeholder, className = "" }: { value: any, onSave: (val: any) => void, type?: string, step?: string, placeholder?: string, className?: string }) => {
    ...
    return (
        <input type={type} step={step} value={localValue} placeholder={placeholder} onChange={(e) => setLocalValue(e.target.value)} onBlur={handleBlur} onKeyDown={handleKeyDown}
            className={`w-full border-none text-center bg-slate-100 focus:ring-2 focus:ring-blue-500 rounded font-black h-9 py-1 text-slate-800 placeholder:font-normal placeholder:text-slate-400 text-[10px] sm:text-xs leading-none ${className}`}
        />
    );
};
...
const NodeContainer = ({ children, selected, title, icon, colorClass, className = '', onDuplicate, onDelete, cost, isTotal, targetGenerators }: any) => {
    return (
        <div className={`rounded-2xl bg-white shadow-xl border-2 transition-all duration-300 min-w-[280px] sm:min-w-[320px] overflow-hidden ${selected ? 'border-blue-500 ring-8 ring-blue-500/10' : colorClass} ${className}`}>
            <div className={`px-3 sm:px-4 py-2 sm:py-3 border-b flex items-center gap-2 sm:gap-3 ${selected ? 'bg-blue-50' : 'bg-slate-50'} cursor-grab active:cursor-grabbing react-flow__node-draghandle`}>
                 <div className="flex-shrink-0">{icon}</div>
                 <div className="flex-grow flex flex-col">
                    <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.15em] text-slate-900">{title}</span>
                    {targetGenerators && targetGenerators.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                            {targetGenerators.map((t: string, i: number) => (
                                <span key={i} className="text-[7px] font-black bg-slate-200 text-slate-600 px-1 rounded uppercase truncate max-w-[80px]">
                                    {t}
                                </span>
                            ))}
                        </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {data.dimensions?.map(dim => (
                                <tr key={dim.id} className="hover:bg-slate-50 h-10 relative">
                                    ...
                                    <td className="p-1 text-center"><button onClick={() => { if (dim.id) data.deleteDimension?.(dim.id, dim); }} className="text-red-300 hover:text-red-500 font-bold text-lg">×</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                        ...
                        <table className="w-full text-[10px] sm:text-[11px]">
                            <thead className="bg-emerald-600 text-white font-black uppercase sticky top-0">
                                <tr>
                                    ...
                                    <th className="p-2 w-16 text-center">CONS</th>
                                    <th className="p-2 w-20 text-center text-blue-200">USIN. CAB</th>
                                    <th className="p-2 w-20 text-center text-blue-200">USIN. CORPO</th>
                                    <th className="p-2 w-20 text-center">CUSTO</th>
                                    <th className="p-2 w-12 text-center">RAMIF</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-emerald-50">
                                {data.dimensions?.map(dim => {
                                    const mat = matOptions.find((m: any) => m.id === dim.baseMaterialId);
                                    const materialCost = mat ? calculateMaterialCost(mat, dim.consumption || 0) : 0;
                                    const machiningCost = (Number(dim.headMachiningCost) || 0) + (Number(dim.bodyPieceCost) || 0);
                                    const rowCost = materialCost + machiningCost;
                                    ...
                                    return (
                                        <tr key={dim.id} className="h-10 relative">
                                            ...
                                            <td className="p-1 bg-blue-50/30"><TableInput value={dim.headMachiningCost || 0} onSave={(val) => data.updateDimension?.(dim.id, { headMachiningCost: val })} /></td>
                                            <td className="p-1 bg-blue-50/30"><TableInput value={dim.bodyPieceCost || 0} onSave={(val) => data.updateDimension?.(dim.id, { bodyPieceCost: val })} /></td>
...
export const ProductGeneratorNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => {
    // Sync logic: if label is generic, we show it but we'll use activeFamiliaName if provided
    const displayLabel = (data.label === "GERADOR FINAL" || data.label === "GERADOR DE PEÇA" || !data.label) 
        ? (data.activeFamiliaName || data.label || "GERADOR DE PEÇA") 
        : data.label;

    // Template Resolution for Preview
    let resolvedName = data.generationConfig?.nameTemplate || '';
    let resolvedSku = data.generationConfig?.skuTemplate || '';
    
    if (data.inventoryContext?.variables) {
        Object.entries(data.inventoryContext.variables).forEach(([k, v]) => {
            const regex = new RegExp(`{${k}}`, 'gi');
            resolvedName = resolvedName.replace(regex, String(v || ''));
            resolvedSku = resolvedSku.replace(regex, String(v || ''));
        });
    }
    ...
    return (
        <div className="relative">
            ...
            <NodeContainer 
                selected={selected} 
                title={displayLabel} 
                colorClass="border-orange-400 bg-orange-50/5" 
                isTotal 
                cost={data.cost} 
                onDuplicate={data.duplicateNode} 
                onDelete={data.deleteNode} 
                targetGenerators={data.targetGenerators} 
                icon={<svg className="h-5 w-5 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>}
            >
                <div className="nodrag transition-all space-y-4">
                    <p className="text-[9px] font-black text-slate-400 uppercase text-center mb-2 italic">CONECTE O DNA NO TOPO E OS PROCESSOS NA ESQUERDA</p>
                    
                    <div className="space-y-4">
                        <Input 
                            label="NOME DO GERADOR" 
                            value={data.label || ''} 
                            onChange={e => data.updateNodeLabel?.(e.target.value)} 
                            placeholder={data.activeFamiliaName || "Ex: Gerador Cabo"} 
                            className="font-bold"
                        />
                        ...
                        <div className="space-y-2">
                            <div>
                                <Input 
                                    label="TEMPLATE NOME ITEM" 
                                    value={data.generationConfig?.nameTemplate || ''} 
                                    onChange={e => data.updateNodeGenerationConfig?.({ nameTemplate: e.target.value })} 
                                    placeholder="Ex: Item M{bitola}" 
                                />
                                {resolvedName && resolvedName !== data.generationConfig?.nameTemplate && (
                                    <div className="mt-1 px-2 py-1 bg-emerald-50 rounded border border-emerald-100 text-[9px] font-bold text-emerald-700 truncate shadow-sm">
                                        GERANDO: {resolvedName}
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                <Input 
                                    label="TEMPLATE SKU ITEM" 
                                    value={data.generationConfig?.skuTemplate || ''} 
                                    onChange={e => data.updateNodeGenerationConfig?.({ skuTemplate: e.target.value })} 
                                    placeholder="Ex: SKU-{bitola}" 
                                />
                                {resolvedSku && resolvedSku !== data.generationConfig?.skuTemplate && (
                                    <div className="mt-1 px-2 py-1 bg-emerald-50 rounded border border-emerald-100 text-[9px] font-bold text-emerald-700 truncate shadow-sm">
                                        SKU: {resolvedSku}
                                    </div>
                                )}
                        ...
                        <Button 
                            size="sm" 
                            onClick={data.onViewGeneratedProducts} 
                            className="w-full h-10 font-bold text-[10px] uppercase bg-slate-900 hover:bg-black shadow-lg rounded-xl flex items-center justify-center gap-2"
                        >
                            <Box size={14} />
                            Ver Itens Gerados
                        </Button>
...
export const FabricationFormUI = ({ data, onUpdateLabel, onUpdateDetails }: any) => {
    const mode = data.costCalculationMode || 'time';
    const selectedOp = data.operations?.find((o: any) => o.id === data.operationId);
    const displayTime = data.manualTimeSeconds !== undefined ? data.manualTimeSeconds : (selectedOp?.timeSeconds || 0);

    return (
        <div className="space-y-4">
            <Input label="NOME DA ETAPA" value={data.label || ''} onChange={e => onUpdateLabel(e.target.value)} />
            
            <Select label="MODO DE CUSTO" value={mode} onChange={e => onUpdateDetails({ costCalculationMode: e.target.value as any })}>
            ...
            {mode === 'fixed' && (
                <Input label="CUSTO FIXO (R$)" type="number" value={data.fixedCost || 0} onChange={e => onUpdateDetails({ fixedCost: isNaN(parseFloat(e.target.value)) ? 0 : parseFloat(e.target.value) })} />
            )}
            
            {mode === 'time' && (
                <div className="space-y-3">
                    <Select label="SERVIÇO" value={data.operationId || ''} onChange={e => onUpdateDetails({ operationId: e.target.value })}><option value="">-- MANUAL --</option>{data.operations?.map((o: any) => <option key={o.id} value={o.id}>{o.name}</option>)}</Select>
                    <div className="grid grid-cols-2 gap-3">
                        <Select label="OPERADOR" value={data.manualOperatorId || ''} onChange={e => onUpdateDetails({ manualOperatorId: e.target.value })}><option value="">PADRÃO</option>{data.operators?.map((ws: any) => <option key={ws.id} value={ws.id}>{ws.name}</option>)}</Select>
                        <Input label="TEMPO (s)" type="number" value={displayTime} onChange={e => onUpdateDetails({ manualTimeSeconds: isNaN(parseFloat(e.target.value)) ? 0 : parseFloat(e.target.value) })} />
                    </div>
                </div>
            )}

            {mode === 'workstation' && (
                <Select label="POSTO DE TRABALHO" value={data.workStationId || ''} onChange={e => onUpdateDetails({ workStationId: e.target.value })}><option value="">ESCOLHER...</option>{data.operators?.map((ws: any) => <option key={ws.id} value={ws.id}>{ws.name}</option>)}</Select>
            )}
        </div>
    );
};
...
export const FabricationNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => {
    return (
        <div className="relative">
            ...
            <NodeContainer selected={selected} title="ETAPA DE PROCESSO" colorClass="border-slate-200" cost={data.cost} onDuplicate={data.duplicateNode} onDelete={data.deleteNode} targetGenerators={data.targetGenerators}>
                <div className="nodrag">
                    <FabricationFormUI 
                        data={data} 
                        onUpdateLabel={data.updateNodeLabel} 
                        onUpdateDetails={data.updateNodeOperationDetails} 
                    />
                </div>
            </NodeContainer>
...
export const MaterialFormUI = ({ data, onUpdateDetails }: any) => {
    const selectedComponent = data.getMaterialOptions?.().find((c: any) => c.id === data.baseMaterialId);
    const unitLabel = selectedComponent ? 
        (selectedComponent.unitCategory === 'length' ? 'M' : 
         selectedComponent.unitCategory === 'weight' ? 'Kg' : 'Un') : 'Un';

    return (
        <div className="space-y-4">
            <Select label="ITEM" value={data.baseMaterialId || ''} onChange={e => onUpdateDetails({ baseMaterialId: e.target.value })}>
                <option value="">ESCOLHER...</option>
                {data.getMaterialOptions?.().map((m: any) => <option key={m.id} value={m.id}>{m.sku}</option>)}
            </Select>
            <Input label={`CONSUMO (em ${unitLabel})`} type="number" step="0.001" value={data.consumption || ''} onChange={e => onUpdateDetails({ consumption: parseFloat(e.target.value) || 0 })} />
        </div>
    );
};
...
export const MaterialNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => (
    <div className="relative">
        <Handle type="target" position={Position.Left} style={handleStyle} />
        <NodeContainer selected={selected} title="MATÉRIA-PRIMA" colorClass="border-emerald-200" cost={data.cost} onDuplicate={data.duplicateNode} onDelete={data.deleteNode} targetGenerators={data.targetGenerators}>
            <div className="nodrag">
                <MaterialFormUI data={data} onUpdateDetails={(details: any) => data.updateNodeMaterialDetails?.(details, [])} />
            </div>
        </NodeContainer>
        <Handle type="source" position={Position.Right} style={handleStyle} />
    </div>
...
export const InventoryFormUI = ({ data, onUpdateDetails }: any) => {
    const isDynamic = !!data.componentIdTemplate || !!data.sourceFamiliaId;
    const selectedComponent = !isDynamic ? data.getInventoryComponentOptions?.().find((c: any) => c.id === data.componentId) : null;
    const unitLabel = selectedComponent ? 
        (selectedComponent.unitCategory === 'length' ? 'M' : 
         selectedComponent.unitCategory === 'weight' ? 'Kg' : 'Un') : 'Un';
    ...
    return (
        <div className="space-y-4">
                ...
                <select 
                    ...
                    onChange={e => {
                        if (e.target.value === 'fixed') {
                            onUpdateDetails({ componentIdTemplate: '', sourceFamiliaId: '' });
                        } else {
                            onUpdateDetails({ componentId: '', componentIdTemplate: '{sku}' });
                        }
                    }}
                >
            ...
            {isDynamic ? (
                        ...
                        <div>
                            ...
                            <select 
                                ...
                                onChange={e => {
                                    ...
                                    const fam = data.allFamilias?.find((f: any) => f.id === famId);
                                    const genNode = fam?.nodes.find((n: any) => n.data.type === 'productGenerator' || n.data.type === 'productGeneratorNode');
                        ...
                        {data.sourceFamiliaId && (
                            <div className="mt-2 p-2 bg-white rounded border border-blue-100">
                                <p className="text-[9px] text-slate-600 leading-relaxed">
                                    As bitolas e comprimentos serão herdados automaticamente deste processo.
                                </p>
                                <div className="mt-2 pt-2 border-t border-blue-50 flex flex-col gap-1">
                                    ...
                                    <input 
                                        ...
                                        onChange={e => onUpdateDetails({ componentIdTemplate: e.target.value })}
                                        placeholder="Ex: PAR-M{bitola}X{comprimento}"
                                    />
                                </div>
                                
                                <div className="mt-3 pt-3 border-t border-blue-50 flex flex-col gap-2">
                                    <div className="flex items-center justify-between">
                                        <label className="text-[8px] font-bold text-slate-400 uppercase">Tradução de Variáveis</label>
                                        <button 
                                            onClick={() => {
                                                const mappings = data.variableMappings || [];
                                                onUpdateDetails({ variableMappings: [...mappings, { id: Date.now().toString(), sourceVar: 'bitola', sourceValue: '', targetValue: '' }] });
                                            }}
                                            className="text-[8px] font-bold text-blue-600 hover:bg-blue-100 bg-white px-1.5 py-0.5 rounded border border-blue-200"
                                        >+ REGRA</button>
                                    </div>
                                    {data.variableMappings?.map((mapping: any) => (
                                        <div key={mapping.id} className="flex gap-1 items-center bg-white p-1 rounded border border-blue-100">
                                            <select 
                                                className="w-14 text-[8px] p-1 border border-slate-200 rounded" 
                                                value={mapping.sourceVar} 
                                                onChange={e => {
                                                    const newMappings = data.variableMappings.map((m: any) => m.id === mapping.id ? { ...m, sourceVar: e.target.value } : m);
                                                    onUpdateDetails({ variableMappings: newMappings });
                                                }}
                                            >
                                                <option value="bitola">Bit.</option>
                                                <option value="comprimento">Comp.</option>
                                            </select>
                                            ...
                                            <input 
                                                ...
                                                onChange={e => {
                                                    const newMappings = data.variableMappings.map((m: any) => m.id === mapping.id ? { ...m, sourceValue: e.target.value } : m);
                                                    onUpdateDetails({ variableMappings: newMappings });
                                                }}
                                            />
                                            <span className="text-[8px] text-slate-400">➜</span>
                                            <input 
                                                className="flex-1 text-[8px] p-1 border border-slate-200 rounded" 
                                                placeholder="15.8" 
                                                value={mapping.targetValue}
                                                onChange={e => {
                                                    const newMappings = data.variableMappings.map((m: any) => m.id === mapping.id ? { ...m, targetValue: e.target.value } : m);
                                                    onUpdateDetails({ variableMappings: newMappings });
                                                }}
                                            />
                                            <button 
                                                onClick={() => {
                                                    const newMappings = data.variableMappings.filter((m: any) => m.id !== mapping.id);
                                                    onUpdateDetails({ variableMappings: newMappings });
                                                }}
                                                className="w-4 flex text-red-400 hover:text-red-600 font-bold justify-center"
                                            >×</button>
                                        </div>
                                    ))}
                                    {(!data.variableMappings || data.variableMappings.length === 0) && (
                                        <div className="text-[8px] text-slate-400 italic text-center p-1">Nenhuma regra local (valores passados 1:1)</div>
                                    )}
                                </div>
                            </div>
                        )}
                ...
                <Select value={data.componentId || ''} onChange={e => onUpdateDetails({ componentId: e.target.value, componentIdTemplate: '', sourceFamiliaId: '' })}>
                    <option value="">SELECIONAR...</option>
                    {(data.getInventoryComponentOptions?.() || []).map((c: any) => <option key={c.id} value={c.id}>{c.sku}</option>)}
                </Select>
            )}
            <Input label={`QUANTIDADE (em ${unitLabel})`} type="number" step="0.001" value={data.consumption || 1} onChange={e => onUpdateDetails({ consumption: parseFloat(e.target.value) || 1 })} />
        </div>
    );
};
...
export const InventoryNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => {
    ...
    // Attempt to resolve SKU for display
    let resolvedSku = data.componentIdTemplate || '';
    if (isDynamic && data.inventoryContext?.variables) {
        let mappedVariables = { ...(data.inventoryContext.variables || {}) };
        ...
        if (data.variableMappings && Array.isArray(data.variableMappings)) {
            data.variableMappings.forEach((mapping: any) => {
                ...
                const currVal = String(mappedVariables[sVarStr] || '').toLowerCase().trim();
                const normalizeM = (s: string) => s.replace(/^m/i, '');
                
                if (currVal === sValStr || normalizeM(currVal) === normalizeM(sValStr)) {
                    mappedVariables[sVarStr] = mapping.targetValue;
                }
            });
        }

        Object.entries(mappedVariables).forEach(([k, v]) => {
            resolvedSku = resolvedSku.replace(new RegExp(`{${k}}`, 'gi'), String(v));
        });
    }
    ...
    return (
        <div className="relative">
            ...
            <NodeContainer 
                selected={selected} 
                title={isDynamic ? "PROCESSO VINCULADO" : "ITEM DE ESTOQUE"} 
                colorClass={isDynamic ? "border-blue-400 bg-blue-50/10" : "border-purple-200"} 
                cost={data.cost} 
                onDuplicate={data.duplicateNode} 
                onDelete={data.deleteNode} 
                targetGenerators={data.targetGenerators}
                icon={isDynamic ? <span>🔗</span> : <span>📦</span>}
            >
                <div className="nodrag transition-all">
                    {isDynamic && resolvedSku && (
                        <div className="mb-3 p-2 bg-white rounded-lg border border-blue-200 shadow-sm">
                            <span className="text-[8px] font-black text-blue-400 uppercase tracking-widest block mb-1">SKU RESOLVIDO</span>
                            <span className="text-[10px] font-black text-blue-900 block truncate font-mono">{resolvedSku}</span>
                        </div>
                    )}
                    <InventoryFormUI data={data} onUpdateDetails={(details: any) => data.updateNodeComponentDetails?.(details, [])} />
                </div>
            </NodeContainer>
            <Handle type="source" position={Position.Right} style={handleStyle} />
        </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {data.headCodes?.map(hc => (
                                <tr key={hc.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="p-1 text-center"><button onClick={() => { if (hc.id) data.deleteHeadCode?.(hc.id, hc); }} className="text-red-300 hover:text-red-500 font-bold text-lg">×</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
export const FinalNode = memo(({ data, selected }: any) => {
    const displayLabel = (!data.label || data.label === "RESULTADO" || data.label === "GERADOR FINAL") 
        ? (data.activeFamiliaName || "RESULTADO") 
        : data.label;
    ...
    return (
        <div className="relative">
            ...
            <NodeContainer 
                selected={selected} 
                title={displayLabel} 
                colorClass="border-slate-900 bg-slate-900 text-white" 
                cost={data.cost} 
                isTotal 
                onDuplicate={data.duplicateNode} 
                onDelete={data.deleteNode}
            ...
            >
                <div className="nodrag">
                    <p className="text-[10px] opacity-60 italic">Soma integral calculada para este ponto final.</p>
                </div>
            </NodeContainer>
        </div>
    );
...
export const VariableNode = memo(({ data, selected }: any) => (
    <div className="relative"><NodeContainer selected={selected} title="VARIÁVEL" colorClass="border-slate-200" onDuplicate={data.duplicateNode} onDelete={data.deleteNode}><div className="nodrag"><Input label="NOME" value={data.label || ''} onChange={e => data.updateNodeLabel?.(e.target.value)} className="mb-3" /><Input label="VALOR" type="number" value={data.cost || 0} onChange={e => data.updateNodeCost?.(e.target.value)} /></div></NodeContainer><Handle type="source" position={Position.Right} style={dataHandleStyle} /></div>
));
