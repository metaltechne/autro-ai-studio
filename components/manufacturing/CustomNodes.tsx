...
export const ProductGeneratorNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => (
    <div className="relative">
        ...
        <NodeContainer selected={selected} title={data.label || "GERADOR FINAL"} colorClass="border-orange-300" isTotal cost={data.cost} onDuplicate={data.duplicateNode} onDelete={data.deleteNode} targetGenerators={data.targetGenerators} icon={<svg className="h-5 w-5 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>}>
            <div className="nodrag">
                ...
                <div className="space-y-4">
                    <Input label="NOME DO GERADOR" value={data.label || ''} onChange={e => data.updateNodeLabel?.(e.target.value)} placeholder="Ex: Gerador Cabo" />
                    <Input label="TEMPLATE NOME ITEM" value={data.generationConfig?.nameTemplate || ''} onChange={e => data.updateNodeGenerationConfig?.({ nameTemplate: e.target.value })} placeholder="Ex: Item M{bitola}" />
                    <Input label="TEMPLATE SKU ITEM" value={data.generationConfig?.skuTemplate || ''} onChange={e => data.updateNodeGenerationConfig?.({ skuTemplate: e.target.value })} placeholder="Ex: SKU-{bitola}" />
...
export const FabricationFormUI = ({ data, onUpdateLabel, onUpdateDetails }: any) => {
    ...
    return (
        <div className="space-y-4">
            <Input label="NOME DA ETAPA" defaultValue={data.label} onBlur={e => onUpdateLabel(e.target.value)} />
            ...
            {mode === 'fixed' && (
                <Input label="CUSTO FIXO (R$)" type="number" value={data.fixedCost || 0} onChange={e => onUpdateDetails({ fixedCost: parseFloat(e.target.value) })} />
            )}
            ...
            {mode === 'time' && (
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <Input label="TEMPO (s)" type="number" value={data.manualTimeSeconds || 0} onChange={e => onUpdateDetails({ manualTimeSeconds: parseFloat(e.target.value) })} />
                    </div>
                </div>
            )}
...
export const MaterialFormUI = ({ data, onUpdateDetails }: any) => {
    return (
        <div className="space-y-4">
            ...
            <Input label="CONSUMO" type="number" defaultValue={data.consumption || 0} onBlur={e => onUpdateDetails({ consumption: parseFloat(e.target.value) || 0 })} />
        </div>
    );
};
...
export const InventoryNode: React.FC<NodeProps<ProcessNodeData>> = memo(({ data, selected }) => {
    return (
        <div className="relative">
            <Handle type="target" position={Position.Left} style={handleStyle} />
            <NodeContainer selected={selected} title="ITEM DE ESTOQUE" colorClass="border-purple-200" cost={data.cost} onDuplicate={data.duplicateNode} onDelete={data.deleteNode} targetGenerators={data.targetGenerators}>
                <div className="nodrag">
...
export const FinalNode = memo(({ data, selected }: any) => (
    <div className="relative">
        ...
        >
            <p className="text-[10px] opacity-60 italic">Soma integral calculada.</p>
        </NodeContainer>
    </div>
));
