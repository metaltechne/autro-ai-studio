                ...
                <AnimatePresence mode="wait">
                                                ...
                                                <tr key={op.id} className="group hover:bg-slate-50/50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        ...
                                                        {isLinkingInsumo === op.id && (
                                                            <div className="absolute z-30 mt-2 w-56 bg-white shadow-2xl border border-slate-200 rounded-2xl p-3 animate-fade-in ring-4 ring-slate-900/5">
                                                                <div className="flex justify-between items-center mb-3">
                                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Base de Insumos</p>
                                                                    <button onClick={() => setIsLinkingInsumo(null)} className="text-slate-300 hover:text-slate-600">×</button>
                                                                </div>
                                                                <div className="max-h-48 overflow-y-auto space-y-1 custom-scrollbar pr-1">
                                                                    {consumables.map(c => (
                                                                        <button 
                                                                            ...
                                                                            className="w-full text-left p-2 hover:bg-slate-50 text-xs rounded-xl font-bold text-slate-600 truncate flex items-center gap-2 group/btn"
                                                                        >
                                                                            <ChevronRight className="w-3 h-3 text-slate-300 group-hover/btn:text-sky-500 transition-colors" />
                                                                            {c.name}
                                                                        </button>
                                                                    ))}
                                                                </div>
                    ...
                    {activeTab === 'workstations' && (
                        <motion.div 
                            ...
                            className="space-y-6"
                        >
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-inner items-end">
                                <Input 
                                    label="Nome do Operador / Posto" 
                                    placeholder="Ex: Célula de Solda" 
                                ...
                                <Input 
                                    ...
                                    className="bg-white"
                                />
                                <Button onClick={handleAddStation} className="h-12 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl shadow-lg flex items-center justify-center gap-2">
                                    <Plus className="w-4 h-4" />
                                    <span className="font-black text-[11px] uppercase">Novo Operador</span>
                                </Button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {workStations.filter(ws => ws.name.toLowerCase().includes(searchTerm.toLowerCase())).map(ws => (
                                    <motion.div 
                                        layout
                                        key={ws.id} 
                                        className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-all group relative overflow-hidden"
                                    >
                                        <div className="absolute top-0 right-0 p-3 flex gap-2 translate-y-[-100%] group-hover:translate-y-0 transition-transform">
                                            <button 
                                                onClick={() => saveWorkStations(workStations.filter(w => w.id !== ws.id))}
                                                className="p-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 shadow-sm border border-red-100 transition-colors"
                                            >
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-slate-800 text-sm">{ws.name}</h4>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">Posto de Trabalho</p>
                                            </div>
                                        </div>
                                        <div className="mt-6 flex items-baseline gap-1">
                                            <span className="text-2xl font-black text-slate-900">{formatCurrency(ws.hourlyRate)}</span>
