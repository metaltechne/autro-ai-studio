                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {workStations.filter(ws => ws.name.toLowerCase().includes(searchTerm.toLowerCase())).map(ws => {
                                    ...
                                    const opStats = performanceByOperator[ws.id] || { produced: 0, actualTime: 1, predictedTime: 1, breakdown: {} };
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4">
                                                        ...
                                                        <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                            ...
                                                            <p className="text-sm font-black text-slate-700">{Object.keys(opStats.breakdown).length} <span className="text-[10px] text-slate-400">tipos</span></p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    >
                                                        <div className="p-6 space-y-4">
                                                            ...
                                                            <div className="space-y-3">
                                                                {Object.entries(opStats.breakdown).slice(0, 10).map(([piece, data]) => {
                                                                    const pieceEfficiency = data.predictedTime > 0 
                                                                        ? Math.min(200, Math.round((data.predictedTime / data.actualTime) * 100))
                                                                        : 100;
                                                                    
                                                                    return (
                                                                        <div key={piece} className="space-y-1">
                                                                            <div className="flex justify-between text-[10px] font-bold text-white">
                                                                                <span className="truncate max-w-[200px]" title={piece}>{piece}</span>
                                                                                <span className={pieceEfficiency >= 100 ? "text-emerald-400" : "text-sky-300"}>
                                                                                    {pieceEfficiency}%
                                                                                </span>
                                                                            </div>
                                                                            <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                                                                                <div 
                                                                                    className={`h-full transition-all duration-1000 ${pieceEfficiency >= 100 ? 'bg-emerald-500' : 'bg-sky-500'}`} 
                                                                                    style={{ width: `${Math.min(100, pieceEfficiency)}%` }} 
                                                                                />
                                                                            </div>
                                                                            <div className="flex justify-between text-[8px] text-white/30">
                                                                                <span>{data.produced} peças produzidas</span>
                                                                                <span>{Math.round(data.actualTime/60)}min gastos</span>
                                                                            </div>
                                                                        </div>
                                                                    );
                                                                })}
                                                                {Object.keys(opStats.breakdown).length === 0 && (
