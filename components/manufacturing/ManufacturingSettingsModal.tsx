...
export const ManufacturingSettingsModal: React.FC<ManufacturingSettingsModalProps> = ({ isOpen, onClose, manufacturing, manufacturingOrdersHook, inventory }) => {
    ...
    const handleUpdateConsumable = (id: string, field: keyof Consumable, value: any) => {
        const updated = consumables.map(c => {
            ...
            const price = field === 'purchasePrice' ? Number(cleanValue) : updatedItem.purchasePrice;
            const cons = field === 'monthlyConsumption' ? Number(cleanValue) : updatedItem.monthlyConsumption;
            const prod = field === 'monthlyProduction' ? Number(cleanValue) : updatedItem.monthlyProduction;
    ...
    const handleAddConsumable = () => {
        ...
        const price = Number(newConsumable.purchasePrice.toString().replace(',', '.')) || 0;
        const cons = Number(newConsumable.monthlyConsumption.toString().replace(',', '.')) || 0;
        const prod = Number(newConsumable.monthlyProduction.toString().replace(',', '.')) || 3000;
    ...
    const handleAddStation = () => {
        ...
        let hRate = parseFloat(newStation.hourlyRate.toString().replace(',', '.')) || 0;

        // Auto-calculate if salary and hours provided
        if (hRate === 0 && newStation.monthlySalary) {
            const salary = parseFloat(newStation.monthlySalary.replace(',', '.'));
            const hours = parseFloat(newStation.workingHoursPerMonth.replace(',', '.')) || 176;
            if (salary > 0 && hours > 0) {
                hRate = salary / hours;
            }
        }
        ...
        saveWorkStations([...workStations, { 
            ...
            monthlySalary: parseFloat(newStation.monthlySalary.replace(',', '.')) || 0,
            workingHoursPerMonth: parseFloat(newStation.workingHoursPerMonth.replace(',', '.')) || 176
        }]);
                    ...
                    {activeTab === 'workstations' && (
                        ...
                        >
                            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-8 gap-3 mb-8 bg-slate-50 p-4 rounded-xl border border-slate-100">
                                    ...
                                    <div className="lg:col-span-1">
                                        <Input 
                                            label="Custo Hora (R$)" 
                                            type="text"
                                            placeholder={newStation.monthlySalary ? "Calculado..." : "Ex: 29,50"}
                                    ...
                                    <table className="min-w-full divide-y divide-slate-100">
                                        <thead className="bg-slate-50">
                                            <tr>
                                                ...
                                                <th className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Salário / Horas</th>
                                                ...
                                                <tr key={ws.id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 text-right">
                                                        <div className="flex flex-col items-end gap-1">
                                                            <input 
                                                                className="text-xs font-mono text-slate-600 bg-transparent border border-slate-200 hover:border-slate-300 rounded px-2 py-0.5 w-24 text-right focus:bg-white focus:outline-none" 
                                                                value={(ws.monthlySalary || 0).toString().replace('.', ',')}
                                                                onChange={e => {
                                                                    const val = parseFloat(e.target.value.replace(',', '.')) || 0;
                                                                    const newStations = workStations.map(w => w.id === ws.id ? {...w, monthlySalary: val} : w);
                                                                    saveWorkStations(newStations);
                                                                }}
                                                                onBlur={() => {
                                                                    // Auto-recalc hourly rate if desired
                                                                    const hours = ws.workingHoursPerMonth || 176;
                                                                    const newRate = (ws.monthlySalary || 0) / hours;
                                                                    const newStations = workStations.map(w => w.id === ws.id ? {...w, hourlyRate: newRate} : w);
                                                                    saveWorkStations(newStations);
                                                                }}
                                                            />
                                                            <input 
                                                                className="text-[10px] font-mono text-slate-400 bg-transparent border border-slate-200 hover:border-slate-300 rounded px-2 py-0.5 w-16 text-right focus:bg-white focus:outline-none" 
                                                                value={(ws.workingHoursPerMonth || 176).toString().replace('.', ',')}
                                                                onChange={e => {
                                                                    const val = parseFloat(e.target.value.replace(',', '.')) || 176;
                                                                    const newStations = workStations.map(w => w.id === ws.id ? {...w, workingHoursPerMonth: val} : w);
                                                                    saveWorkStations(newStations);
                                                                }}
                                                            />
                                                        </div>
