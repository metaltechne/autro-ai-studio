...
export const StockMovementView: React.FC<{ inventory: InventoryHook }> = ({ inventory }) => {
    ...
    const startScanner = useCallback(() => {
        if (!isScanningRef.current && window.Html5Qrcode && !isStoppingRef.current) {
            if (!scannerRef.current) {
                try {
            ...
            scanner.start(
             ...
             .catch((err: any) => {
                if (isMounted.current) {
                    ...
                    addToast("Erro na Câmera ou permissão negada.", 'error');
                }
             });
        }
    ...
    const executeConfirm = async () => {
        ...
        try {
            const logs = Array.from(sessionItems.values()).map((item: SessionItem) => ({
                ...
                type: (activeTab === 'entrada' ? 'entrada' : 'saída') as any,
                quantity: item.quantity,
                reason: activeTab === 'entrada' ? 'compra_fornecedor' as any : checkoutReason,
            ...
            addToast("Erro ao processar lote.", 'error');
        } finally {
            ...
            <div className="flex-grow grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
                ...
                <div className="lg:col-span-5 flex flex-col gap-6">
                    <Card className="flex flex-col items-center justify-center p-8 bg-slate-900 border-none shadow-2xl relative overflow-hidden">
                        ...
                        {/* Container do Scanner - Mantido limpo para o Html5Qrcode */}
                        <div className="w-full aspect-square max-w-[320px] rounded-3xl overflow-hidden border-4 border-slate-700 shadow-inner relative bg-black">
                            <div id={scannerContainerId} className="w-full h-full"></div>
                            
                            {/* Overlays gerenciados pelo React FORA do container id do scanner */}
                            {!isScanning && (
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 bg-slate-800/50 backdrop-blur-sm pointer-events-none">
                            ...
                            {isScanning && <div className="absolute top-0 left-0 w-full h-1 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.8)] animate-scan-line z-10 pointer-events-none"></div>}
                        </div>
                ...
                {activeTab !== 'conferencia' && (
                    <Card className="lg:col-span-7 flex flex-col min-h-0 p-0 overflow-hidden border-2 border-slate-200">
                        ...
                        <div className="flex-grow overflow-y-auto p-6 space-y-3 bg-white">
                            {sessionItems.size > 0 ? Array.from(sessionItems.values()).map(({ component, quantity }) => (
