...
import { AIAction, AIWorkerViewProps, ProductionOrderItem } from '../types';
...
- **IA (JSON):** { "functionName": "adicionarEstoque", "params": { "sku": "COPO-STD", "quantidade": 50 }, "displayText": "Você confirma a adição de 50 unidades ao estoque do componente 'Copo Padrão' (SKU: COPO-STD)?", "requiresConfirmation": true }`;
    ...
    const tools: Record<string, Function> = {
        ...
        // Fix: Changed parameter type from 'quantity' to 'quantidade' to be consistent with the prompt and implementation
        criarOrdemDeProducao: async ({ itens }: { itens: {sku: string, quantidade: number}[] }): Promise<string> => {
            ...
            for (const item of itens) {
                ...
                // Fix: Corrected property access to use 'item.quantidade'
                orderItems.push({ kitId: kit.id, quantity: item.quantidade });
            }
            if (!financialSettings) return "Erro: Configurações financeiras não carregadas.";
            const { scenarios, virtualComponents } = analyzeProductionRun(orderItems, [], familias, components, financialSettings);
