...
const SubstitutionSuggestion: React.FC<{ sub: SubstitutionOption; canViewCosts: boolean }> = ({ sub, canViewCosts }) => (
    <div className="text-xs text-blue-800 bg-blue-100 p-2 rounded-md mt-1 border border-blue-200">
        ...
        <p>
            ...
            {canViewCosts && <span className="font-semibold"> Custo Extra: </span>}
            {canViewCosts && formatCurrency(sub.costOfCutting) + '/un | '}
...
}> = ({ scenario, isRecommended, onSelect, isCreating, canViewCosts }) => {
                                ...
                                {(shortage.substitutionOptions || []).length > 0 && (
                                    <>
                                        ...
                                        <div className="space-y-1">
                                            {shortage.substitutionOptions?.map((sub, i) => <SubstitutionSuggestion key={i} sub={sub} canViewCosts={canViewCosts} />)}
                                        </div>
                                    </>
                                )}
                            </div>
