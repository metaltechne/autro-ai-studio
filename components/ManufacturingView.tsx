...
import { Search, Plus, Settings, Copy, Trash2, Edit3, Layout, Layers, Box, Cpu, Database, ChevronRight, Info } from 'lucide-react';
...
const ManufacturingCanvas: React.FC<{
    ...
    const styledEdges = useMemo(() => {
        return edges.map(edge => ({
            ...
            style: {
                ...edge.style,
                stroke: highlightedEdges.has(edge.id) ? '#3b82f6' : '#cbd5e1',
                strokeWidth: highlightedEdges.has(edge.id) ? 3 : 1,
                opacity: highlightedNodes.size > 0 && !highlightedEdges.has(edge.id) ? 0.2 : 1,
                transition: 'all 0.3s ease-in-out'
            }
        }));
    }, [edges, highlightedEdges, highlightedNodes]);
        ...
        <div className="h-full w-full relative bg-slate-50 flex" ref={reactFlowWrapper}>
            ...
            <div className="flex-1 relative">
                <ReactFlow
                    nodes={styledNodes}
                    edges={styledEdges}
                    onNodesChange={manufacturing.onNodesChange(activeFamilia?.id || '')}
                    onEdgesChange={manufacturing.onEdgesChange(activeFamilia?.id || '')}
                    onConnect={manufacturing.onConnect(activeFamilia?.id || '')}
                    nodeTypes={nodeTypes}
                    edgeTypes={edgeTypes}
...
export const ManufacturingView: React.FC<ManufacturingViewProps> = ({ manufacturing, inventory, setCurrentView, onShowQRCode, mode = 'all' }) => {
    ...
    const dnaNode = useMemo(() => {
        return activeFamilia?.nodes.find(n => 
            ...
            n.data.type === 'dimensionTableNode'
        );
    }, [activeFamilia]);
