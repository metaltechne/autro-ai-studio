...
const ManufacturingCanvas: React.FC<{
    ...
    const nodesWithHooks = useMemo(() => {
        ...
        return nodes.map(node => {
            const calculatedNode = evaluationResult?.nodes?.find(n => n.id === node.id);
            const enhancedData: ProcessNodeData = { 
                ...node.data,
                cost: calculatedNode?.cost !== undefined ? calculatedNode.cost : node.data.cost,
                ...
                activeFamiliaName: activeFamilia.nome,
                inventoryContext: {
                    components: inventory.components,
                    updateComponent: inventory.updateComponent,
                    variables: {
                        bitola: selectedDimension?.bitola,
                        comprimento: selectedDimension?.comprimento,
                        ...selectedDimension
                    }
                },
...
export const ManufacturingView: React.FC<ManufacturingViewProps> = ({ manufacturing, inventory, setCurrentView, onShowQRCode, mode = 'all', manufacturingOrdersHook }) => {
    ...
    const evaluationResult = useMemo(() => {
        ...
        if (selectedDimension) {
            ...
            Object.entries(selectedDimension).forEach(([k, v]) => {
                stringVariables[k] = String(v);
            });
            stringVariables.dimensao = `${selectedDimension.bitola}x${selectedDimension.comprimento}`;
        }
    ...
    return (
        <div className="h-[calc(100vh-6rem)] flex flex-col bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
            ...
            {isSettingsModalOpen && <ManufacturingSettingsModal isOpen={isSettingsModalOpen} onClose={() => setIsSettingsModalOpen(false)} manufacturing={manufacturing} manufacturingOrdersHook={manufacturingOrdersHook} inventory={inventory} />}
