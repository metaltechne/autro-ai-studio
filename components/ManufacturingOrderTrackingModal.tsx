...
export const ManufacturingOrderTrackingModal: React.FC<ManufacturingOrderTrackingModalProps> = ({ isOpen, onClose, order, inventory, onSave }) => {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await onSave(order.id, { trackingSteps: steps });
    ...
    const handlePrintStep = async (step: ManufacturingTrackingStep, index: number) => {
        ...
        try {
            ...
            pdf.save(`OP_${order.id.substring(0, 8)}_Etapa_${index + 1}_${step.name.replace(/\s+/g, '_')}.pdf`);
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`Acompanhamento: ${order.id}`} size="xl">
            <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <div>
                        <p className="text-xs font-black text-slate-500 uppercase">Tipo</p>
                        <p className="font-bold text-slate-900 capitalize">{order.type || 'interna'}</p>
                    </div>
                    <div>
                        <p className="text-xs font-black text-slate-500 uppercase">Lote/OP</p>
                        <p className="font-bold text-slate-900">{order.batchNumber || '-'}</p>
                    </div>
                    <div>
