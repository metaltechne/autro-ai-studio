...
export const ManufacturingOrderTrackingModal: React.FC<ManufacturingOrderTrackingModalProps> = ({ isOpen, onClose, order, inventory, onSave, onUpdateItemStatus }) => {
    ...
    const [steps, setSteps] = useState<ManufacturingTrackingStep[]>(() => {
        const rawSteps = order.trackingSteps || [];
        // Filter out steps that are explicitly material or raw material for a cleaner tracked view
        // Legacy steps might not have .type, so we also check if they have 0 time and aren't obviously labor
        return rawSteps.filter(step => {
            if (step.type === 'material' || step.type === 'materiaPrima') return false;
            
            // If no type, it's likely a labor step if it has time > 0
            if (!step.type && step.predictedTimeSeconds === 0) {
                const nameLower = step.name.toLowerCase();
                // If it's a known service name, keep it even with 0 time (might be a cheap service)
                const isService = nameLower.includes('solda') || 
                                 nameLower.includes('usinagem') || 
                                 nameLower.includes('corte') || 
                                 nameLower.includes('dobra') ||
                ...
                <div>
                    ...
                    <div className="space-y-3 mb-8">
                        {orderItems.map((item, idx) => {
                            ...
                            return (
                                <div key={`${item.componentId}-${idx}`} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl shadow-sm">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-slate-900">
                                                {item.quantity}x {component?.name || item.name || item.componentId}
