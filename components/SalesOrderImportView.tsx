...
export const SalesOrderImportView: React.FC<SalesOrderImportViewProps> = ({ inventory, manufacturing, productionOrdersHook, cuttingOrdersHook, manufacturingOrdersHook, purchaseOrdersHook, customersHook, setCurrentView }) => {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const clienteIdx = 0; // O relatório começa com o cliente na primeira coluna
            ...
            lines.slice(1).forEach(line => {
                ...
                // Detecta mudança de cliente: Se for uma linha que começa um novo grupo (tem nome de cliente na primeira coluna)
                if (columns[clienteIdx] && columns[clienteIdx] !== ' ' && columns[clienteIdx] !== 'Totais') {
                    currentCustomer = columns[clienteIdx];
                }

                const sku = columns[headerIndices["Código"]];
                const description = columns[headerIndices["Produto"]];
                const rawQty = columns[headerIndices["Qtde"]];
                
                if (sku && description && rawQty) {
                    const quantity = Math.ceil(parseFloat(rawQty.replace(',', '.')));
            ...
            if (ordersMap.size === 1) {
                try {
                    await handleProcessOrder(ordersMap.keys().next().value);
                } catch (e: any) {
