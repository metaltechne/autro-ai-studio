...
const parseLocalizacao = (composition: string): KitComposition => {
    ...
    (parts || []).forEach(part => {
        ...
        // Skip parts like "10 UN" or "4 PÇS" if they don't have the item type
        const match = trimmedPart.match(/^(\d+)\b.*?\b(FIX|PORCA|POR|FIXADOR|PARAFUSO|PAR)\b.*?M?(\d+(?:[xX]\d+)?)/i);

        if (match) {
            const quantity = parseInt(match[1], 10);
            const typeValue = match[2].toUpperCase();
            const isNut = (typeValue === 'PORCA' || typeValue === 'POR');
...
export const SalesOrderImportView: React.FC<SalesOrderImportViewProps> = ({ inventory, manufacturing, productionOrdersHook, cuttingOrdersHook, manufacturingOrdersHook, purchaseOrdersHook, customersHook, setCurrentView }) => {
    ...
    useEffect(() => {
        if (stage !== 'idle' && stage !== 'done' && stage !== 'error') {
            const stateToSave = {
                stage,
                customerInfo,
                parsedOrders: Array.from(parsedOrders.entries()),
                aggregatedCsvItems: Array.from(aggregatedCsvItems.entries()),
                missingKits,
                kitsToCreate: Array.from(kitsToCreate.entries()),
                finalOrderItems,
                extraItems,
                mismatchWarning
            };
            api.saveLocalDraft('salesImport', stateToSave);
        }
    }, [stage, customerInfo, parsedOrders, aggregatedCsvItems, missingKits, kitsToCreate, finalOrderItems, extraItems, mismatchWarning]);

    // RESTORE EFFECT
    useEffect(() => {
        const draft = api.getLocalDraft('salesImport');
        if (draft && draft.data) {
            const { stage, customerInfo, parsedOrders, aggregatedCsvItems, missingKits, kitsToCreate, finalOrderItems, extraItems, mismatchWarning } = draft.data;
            let restoredStage = stage;
            if (stage === 'analyzing' || stage === 'confirming_creations') {
                restoredStage = (finalOrderItems && finalOrderItems.length > 0) ? 'prompting_for_keys' : 'select_order';
            }
            setStage(restoredStage);
            setCustomerInfo(customerInfo);
            setParsedOrders(new Map(parsedOrders));
            setAggregatedCsvItems(new Map(aggregatedCsvItems));
            setMissingKits(missingKits);
            setKitsToCreate(new Map(kitsToCreate));
            setFinalOrderItems(finalOrderItems);
            setExtraItems(extraItems || []);
            setMismatchWarning(mismatchWarning);
            addToast("Importação restaurada do rascunho anterior.", "info");
        }
    }, []);
    ...
    const resetState = (isNewImport: boolean = false) => {
        if (!isNewImport) {
            ...
            api.clearLocalDraft('salesImport');
        } else {
    ...
    const handleProceedFromKeys = () => {
        if (missingKits.length > 0) {
            setKitsToCreate(new Map(missingKits.map((m: MissingKitData) => [m.sku, { 
                'Nome do Kit': m.name, 
                SKU: m.sku, 
                Marca: m.marca, 
                Modelo: m.modelo, 
                Ano: m.ano, 
                'Componentes (SKU:Qtd)': '', 
                'Fixadores (Dimensao:Qtd)': m.csvComposition.requiredFasteners.map(f => `${f.dimension}:${f.quantity}`).join(','), 
                'Preco de Venda (Opcional)': undefined,
                selectedFamiliaId: globalFastenerFamiliaId || undefined,
                selectedNutFamiliaId: globalNutFamiliaId || undefined
            }])));
    ...
    const renderPromptForKeys = () => (
        <Card>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Família Fixador Global (Substituir Todos)</label>
                    <select 
                        value={globalFastenerFamiliaId} 
                        onChange={e => {
                            const val = e.target.value;
                            setGlobalFastenerFamiliaId(val);
                            setFinalOrderItems(prev => prev.map(it => it.type === 'kit' ? { ...it, selectedFamiliaId: val } : it));
                        }}
                        className="w-full text-xs p-2 border rounded-lg bg-white font-bold"
                    >
                        <option value="">Manter Padrão</option>
                        <option value="fam-MONTAGEM-FIX-S">FIX-S (Original)</option>
                        <option value="fam-MONTAGEM-FIX-P">FIX-P (Inox)</option>
                        <option value="fam-MONTAGEM-FIX-S-EXT">FIX-S EXT</option>
                        <option value="fam-MONTAGEM-FIX-P-SVD">FIX-P (SVD)</option>
                        {manufacturing.familias.filter(f => f.category === 'manufacturing' && !f.id.includes('MONTAGEM')).map(f => (<option key={f.id} value={f.id}>{f.nome}</option>))}
                    </select>
                </div>
                <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Família Porca Global (Substituir Todos)</label>
                    <select 
                        value={globalNutFamiliaId} 
                        onChange={e => {
                            const val = e.target.value;
                            setGlobalNutFamiliaId(val);
                            setFinalOrderItems(prev => prev.map(it => it.type === 'kit' ? { ...it, selectedNutFamiliaId: val } : it));
                        }}
                        className="w-full text-xs p-2 border rounded-lg bg-white font-bold"
                    >
                        <option value="">Manter Padrão</option>
                        <option value="fam-MONTAGEM-POR-P">POR-P (Inox)</option>
                        {manufacturing.familias.filter(f => f.category === 'manufacturing' && f.nome?.toLowerCase().includes('porca') && f.id !== 'fam-MONTAGEM-POR-P').map(f => (<option key={f.id} value={f.id}>{f.nome}</option>))}
                    </select>
                </div>
            </div>
