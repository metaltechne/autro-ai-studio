...
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, GoogleAuthProvider, OAuthProvider, signInWithPopup } from '@firebase/auth';
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/invalid-credential') {
        setError('Credenciais inválidas. Se este é um novo banco de dados, você precisa criar uma conta primeiro.');
    ...
    <div className={containerClasses}>
      ...
      <Card className="w-full max-w-md shadow-2xl">
        ...
        <h2 className="text-2xl font-bold text-center text-black mb-1">
          {isSignUp ? 'Criar Conta' : 'Login do Sistema'}
        </h2>
        <p className="text-center text-gray-500 mb-6">
          {isSignUp ? 'Registre-se para acessar o sistema.' : 'Bem-vindo de volta.'}
        </p>
        ...
        <form onSubmit={handleAuth} className="space-y-6">
          ...
          <Button type="submit" className="w-full" disabled={isLoading}>
            {isLoading ? 'Aguarde...' : (isSignUp ? 'Criar Conta' : 'Entrar com Email')}
          </Button>
        </form>
        ...
        <div className="mt-6 text-center">
          <button 
            ...
            className="text-sm text-autro-primary hover:underline bg-transparent border-none cursor-pointer"
          >
            {isSignUp ? 'Já tem uma conta? Faça login' : 'Novo banco de dados? Crie sua conta'}
          </button>
        </div>
      </Card>
    </div>
