        ...
        <div className="space-y-6">
            ...
            {viewMode === 'grid' ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {paginatedComponents.map((c, index) => {
                        ...
                        return (
                            <Card key={`${c.id}-${index}`} className={`flex flex-col h-full border-t-4 ${isProcessControlled ? 'border-t-purple-500' : 'border-t-slate-900'}`}>
            ...
            {isAddModalOpen && <ComponentAddModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSave={async (comp, initialStock) => {
                ...
                if (added && initialStock > 0) {
                    await addInventoryLog({ componentId: added.id, type: 'entrada', quantity: initialStock, reason: 'estoque_inicial', notes: 'Estoque inicial inserido no cadastro.' });
                }
                return added;
            }} familias={familias} />}
            {isEditModalOpen && editingComponent && <ComponentEditModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSave={async (comp, stockDelta) => {
                ...
                if (stockDelta && stockDelta !== 0) {
                    await addInventoryLog({ componentId: comp.id, type: stockDelta > 0 ? 'entrada' : 'saída', quantity: Math.abs(stockDelta), reason: stockDelta > 0 ? 'ajuste_inventario_positivo' : 'ajuste_inventario_negativo', notes: 'Ajuste manual durante edição.' });
                }
...
const ComponentAddModal: React.FC<{
    ...
    const handleSave = async () => {
        ...
        await onSave(data as Omit<Component, 'id' | 'stock'>, initialStock);
        onClose();
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Adicionar Novo Componente">
            <div className="space-y-4">
                ...
                <Input label="Quantidade Inicial no Estoque" type="number" min="0" value={initialStock} onChange={e => setInitialStock(parseFloat(e.target.value) || 0)} />
