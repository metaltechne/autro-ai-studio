...
export const ComponentsView: React.FC<ComponentsViewProps> = ({ inventory, manufacturing, setCurrentView, onShowQRCode, initialFilter, onClearFilter }) => {
    ...
    const [filterTab, setFilterTab] = useState<'all' | 'component' | 'raw_material' | 'consumable' | 'low'>('all');
    ...
    const filteredComponents = useMemo(() => {
        ...
        if (filterTab === 'component') results = results.filter(c => c.type === 'component');
        if (filterTab === 'raw_material') results = results.filter(c => c.type === 'raw_material');
        if (filterTab === 'consumable') results = results.filter(c => c.type === 'consumable');
    ...
    }, [components, searchTerm, filterTab]);
        ...
        <div className="space-y-6">
            <header className="flex flex-col md:flex-row justify-between md:items-end gap-4">
                <div>
                    <div className="flex items-center gap-3 mb-1">
                        <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic">Estoque Geral</h2>
                        {isOutdated && (
                    ...
                    <div className="flex items-center gap-3">
                        <p className="text-slate-500 font-medium">Gestão tática e controle de estoque de todos os itens e materiais.</p>
                ...
                <div className="flex items-center gap-2">
                    <Button 
                        onClick={async () => {
                            ...
                            addToast('Todos os custos de itens foram recalculados.', 'success');
                        }} 
                        variant="secondary" 
                    ...
                    <Button onClick={() => setIsAddModalOpen(true)} className="h-11 shadow-lg shadow-blue-500/20 tracking-tighter uppercase font-black italic">+ Novo Item</Button>
                </div>
            </header>

            <Card className="bg-slate-50 border-slate-200">
                <div className="flex flex-col md:flex-row justify-between gap-4 overflow-x-auto pb-2 custom-scrollbar">
                    <div className="flex bg-slate-200/50 p-1 rounded-xl w-fit shrink-0">
                        {[
                            { id: 'all', label: 'Todos' },
                            { id: 'component', label: 'Componentes' },
                            { id: 'raw_material', label: 'Matéria-Prima' },
                            ...
                            <button 
                                ...
                                className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${filterTab === tab.id ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                {tab.label}
                            </button>
                        ))}
                    </div>
                    <div className="flex-grow max-w-md relative shrink-0 min-w-[200px]">
                        <Input 
            ...
            {viewMode === 'grid' ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {paginatedComponents.map((c, index) => {
                        return (
                            <Card key={`${c.id}-${index}`} className={`flex flex-col h-full border-t-4 border-t-slate-900`}>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody className="divide-y divide-slate-100 bg-white">
                                {paginatedComponents.map((c, index) => (
                                    <tr key={`${c.id}-${index}`} className="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => { setSelectedComponent(c); setIsLogModalOpen(true); }} className="p-2 text-slate-400 hover:text-autro-primary" title="Ajustar Estoque"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg></button>
            ...
            {isAddModalOpen && <ComponentAddModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSave={async (comp, initialStock) => {
                const added = await addComponent(comp);
                if (added && initialStock > 0) {
                    const factor = comp.purchaseQuantity || 1;
                    const stockInConsumptionUnit = (comp.sourcing === 'purchased' && factor > 1) 
                        ? initialStock * factor 
                        : initialStock;
                    ...
                    await addInventoryLog({ 
                        ...
                        type: 'entrada', 
                        quantity: stockInConsumptionUnit, 
                        reason: 'estoque_inicial', 
                        notes: 'Estoque inicial inserido no cadastro.' 
                    });
                }
                return added;
            }} familias={familias} />}
            {isEditModalOpen && editingComponent && <ComponentEditModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSave={async (comp, stockDelta) => {
                // Atualiza o componente inteiro, incluindo o novo estoque já calculado
                await updateComponent(comp);
                
                // Se houve mudança no estoque, salvamos apenas um log de histórico
                if (stockDelta && stockDelta !== 0) {
                    const newLog = {
                        id: `log-${Math.random().toString(36).substr(2, 9)}`,
                        componentId: comp.id,
                        type: stockDelta > 0 ? 'entrada' : 'saída',
                        quantity: Math.abs(stockDelta),
                        reason: stockDelta > 0 ? 'ajuste_inventario_positivo' : 'ajuste_inventario_negativo',
                        notes: 'Ajuste manual via edição de componente.',
                        date: new Date().toISOString()
                    } as any;
                    
                    // Salvamos silenciosamente para histórico, sem disparar um novo load nem duplicação
                    const logs = await inventory.getLogsForComponent(comp.id); // Pega apenas para forçar cache, o certo é pegar todos
                    const db = await import('../hooks/api');
                    const allLogs = await db.getInventoryLogs();
                    await db.saveInventoryLogs([...allLogs, newLog]);
                }
            }} componentToEdit={editingComponent} familias={familias} />}
...
const ComponentAddModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSave: (component: Omit<Component, 'id' | 'stock'>, initialStock: number) => Promise<any>;
    ...
    const handleSave = async () => {
        if (!data.name || !data.sku) return alert('Campos obrigatórios faltando (Nome e SKU).');
        ...
        if (data.sourcing === 'purchased' && data.purchaseCost && factor > 0) {
            finalCost = data.purchaseCost / factor;
        }

        const componentToSave = {
            ...data,
            cost: finalCost
        } as Omit<Component, 'id' | 'stock'>;

        console.log('ComponentsView: Adding', {
        ...
        await onSave(componentToSave, initialStock);
        onClose();
    };
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Adicionar Novo Componente">
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <Select label="Origem" value={data.sourcing} onChange={e => setData({...data, sourcing: e.target.value as any})}>
                    ...
                    <Select label="Tipo" value={data.type} onChange={e => setData({...data, type: e.target.value as any})}>
                ...
                {data.sourcing === 'purchased' && (
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input label="Un. Compra (Barra, Kg)" placeholder="Ex: Barra" value={data.purchaseUnit || ''} onChange={e => setData({...data, purchaseUnit: e.target.value})} />
                            <Input label="Un. Consumo (m, mm, Kg)" placeholder="Ex: m" value={data.consumptionUnit || ''} onChange={e => setData({...data, consumptionUnit: e.target.value})} />
                            <Input label="Fator Conversão" type="number" step="0.001" placeholder="Ex: 6" value={data.purchaseQuantity || 1} onChange={e => setData({...data, purchaseQuantity: parseFloat(e.target.value) || 1})} />
                            <Input label="Custo por Un. Compra" type="number" step="0.01" value={data.purchaseCost || ''} onChange={e => setData({...data, purchaseCost: parseFloat(e.target.value) || 0})} />
                        </div>
                        {resultingUnitCost > 0 && (
                ...
                <div className="bg-blue-50 p-3 rounded-xl border border-blue-100">
                    <Input label={`Estoque Inicial (${data.purchaseUnit || 'Unid.'})`} type="number" min="0" value={initialStock} onChange={e => setInitialStock(parseFloat(e.target.value) || 0)} />
                </div>

                <div className="flex justify-end gap-2 pt-4 border-t">
                    <Button variant="secondary" onClick={onClose}>Cancelar</Button>
                    <Button onClick={handleSave}>Cadastrar Componente</Button>
                </div>
            </div>
        </Modal>
    );
