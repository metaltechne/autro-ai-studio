...
export const SettingsView: React.FC<SettingsViewProps> = ({ inventory, manufacturing, campaigns }) => {
    ...
    const [isSyncingSKUs, setIsSyncingSKUs] = useState(false);

    const handleSKUSync = async () => {
        setIsSyncingSKUs(true);
        try {
            await api.forceComponentSKUMigration();
            addToast('Sincronização de SKUs concluída com sucesso! Recarregando...', 'success');
            setTimeout(() => window.location.reload(), 2000);
        } catch (e) {
            addToast('Erro ao sincronizar SKUs.', 'error');
        } finally {
            setIsSyncingSKUs(false);
        }
    };
    ...
    return (
        <div>
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
                    ...
                    {isPermissionsLoading ? (
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {(() => {
                                    const grouped = {
                                        'Dashboards': [View.SECTOR_DASHBOARD, View.OPERATIONAL_DASHBOARD, View.FINANCIAL_DASHBOARD, View.MANUFACTURING_DASHBOARD, View.MACHINE_DASHBOARD, View.ACTIVITY_LOG],
                                        'Gestão de Estoque': [View.COMPONENTS, View.KITS, View.KITS_BY_BRAND, View.KIT_DETAILS, View.INVENTORY_ANALYSIS, View.STOCK_MOVEMENT, View.INSPECTION_RECEIVING, View.LABEL_PRINTING, View.SPREADSHEETS],
                                        'Produção e Engenharia': [View.MANUFACTURING, View.PRODUCTION_PLANNER, View.PRODUCTION_ORDERS, View.MANUFACTURING_PLANNER, View.MANUFACTURING_ORDERS, View.MANUFACTURING_CALENDAR, View.FASTENER_CUTTING, View.CUTTING_ORDERS, View.MANUFACTURING_STRUCTURE, View.MANUFACTURING_CONTROL_CENTER, View.ORDER_VERIFICATION],
                                        'Comercial': [View.SALES_ORDER_IMPORT, View.CUSTOMERS],
                            ...
                            {/* Costs and Fees */}
                             <div className="border-b pb-6">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    ...
                                    <Input label="Outros Custos Variáveis (%)" name="otherVariableCosts" type="number" value={financialForm.otherVariableCosts} onChange={handleFinancialChange} />
                                </div>
                            </div>
                ...
                <Card>
                    <h3 className="text-xl font-semibold text-black mb-2">Manutenção do Sistema</h3>
                    <p className="text-sm text-gray-600 mb-6">
                        Ferramentas de correção e suporte técnico para integridade de dados.
                    </p>
                    <div className="space-y-4">
                        <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl">
                            <h4 className="font-bold text-blue-900 text-sm mb-1 uppercase tracking-tight">Sincronização Forçada de SKUs</h4>
                            <p className="text-xs text-blue-700 leading-relaxed mb-4">
                                Use esta ferramenta se notar discrepâncias entre o inventário e os kits (ex: itens aparecendo como "Desconhecido"). Essa ação irá limpar rascunhos locais e forçar a atualização dos códigos (SKUs) dos componentes baseados na configuração mestre da Engenharia.
                            </p>
                            <Button
                                onClick={async () => {
                                    setIsSyncingSKUs(true);
                                    await inventory.refreshFromCloud();
                                    addToast('Sincronização de dados iniciada.', 'success');
                                    setIsSyncingSKUs(false);
                                }}
                                disabled={isSyncingSKUs}
                                variant="secondary"
                                className="w-full sm:w-auto"
                            >
                                {isSyncingSKUs ? 'Sincronizando...' : 'Sincronizar Estoque Agora'}
                            </Button>
                        </div>

                        <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl">
                            <h4 className="font-bold text-amber-900 text-sm mb-1 uppercase tracking-tight">Limpeza de Cache e Rascunhos</h4>
                            <p className="text-xs text-amber-700 leading-relaxed mb-4">
                                Se você continua vendo dados antigos mesmo após a sincronização, use esta opção para limpar todos os rascunhos locais salvos no seu navegador. Isso forçará o carregamento dos dados diretamente do servidor.
                            </p>
                            <Button
                                onClick={async () => {
                                    await api.hardResetData();
                                    addToast('Cache local limpo! Recarregando...', 'success');
                                }}
                                variant="secondary"
                                className="w-full sm:w-auto"
                            >
                                Limpar Cache Local (Hard Reset)
                            </Button>
                        </div>
                    </div>
                </Card>
