...
import { InventoryHook, ManufacturingHook, ProductionScenario, ProductionOrderItem, Kit, View, Component, ProductionOrdersHook, CuttingOrdersHook, ManufacturingOrdersHook, PurchaseOrdersHook } from '../types';
...
export const FleetPlannerView: React.FC<FleetPlannerViewProps> = ({ inventory, manufacturing, productionOrdersHook, cuttingOrdersHook, manufacturingOrdersHook, purchaseOrdersHook, setCurrentView, createAndStockComponent }) => {
  ...
  const handleAddComponent = (kit: Kit) => {
    ...
    setOrder(prevOrder => {
        const existingItem = prevOrder.kits.find(item => item.id === kit.id);
        if (existingItem) {
            return { ...prevOrder, kits: prevOrder.kits.map(item => item.id === kit.id ? { ...item, quantity: item.quantity + 1 } : item)};
        }
        return { ...prevOrder, kits: [...prevOrder.kits, { id: kit.id, type: 'kit', quantity: 1 }]};
    });
  };
  ...
  const handleAddAllFromFleet = useCallback(() => {
    ...
    setOrder(prevOrder => {
        ...
        prevOrder.kits.forEach(item => newKitsMap.set(item.id, { ...item }));
        ...
        availableKitsForSelection.forEach(kitToAdd => {
            ...
            if (existingItem) {
                ...
                newKitsMap.set(kitToAdd.id, { id: kitToAdd.id, type: 'kit', quantity: 1 });
            }
        });
  ...
  const handleKitQuantityChange = (kitId: string, quantity: number) => {
    ...
    setOrder(prevOrder => ({
        ...
        kits: prevOrder.kits.map(item => 
            item.id === kitId ? { ...item, quantity: Math.max(1, quantity) } : item
        )
    }));
  };
  ...
  const handleRemoveKitFromOrder = (kitIdToRemove: string) => {
      ...
      setOrder(prevOrder => ({
          ...prevOrder,
          kits: prevOrder.kits.filter(item => item.id !== kitIdToRemove)
      }));
  }
    ...
    const KitsContent = (
        <div className="flex flex-col gap-4 h-full">
            ...
            <div className="flex-grow overflow-y-auto space-y-2 min-h-0">
                {availableKitsForSelection.map((kit, index) => (
                    <div key={`${kit.id}-${index}`} className="p-2 border rounded-md bg-white hover:bg-gray-50 flex items-center justify-between gap-2 touch-pan-y">
        ...
        <Card className="lg:col-span-2 flex flex-col overflow-hidden h-full">
            ...
            <div className="flex-grow flex flex-col min-h-0">
                <div className="flex-grow border-2 border-dashed rounded-lg transition-colors overflow-y-auto p-4 space-y-3 mb-4 border-gray-300">
                    ...
                    <>
                         {order.kits.map((item: ProductionOrderItem, index) => {
                            const kit = findKitById(item.id); if (!kit) return null;
                            return (<div key={`${item.id}-${index}`} className="p-3 bg-white rounded-md shadow-sm border flex justify-between items-center gap-4">
                                ...
                                <div className="flex items-center gap-2"><Input type="number" value={item.quantity} onChange={(e) => handleKitQuantityChange(item.id, parseInt(e.target.value) || 1)} min="1" className="w-20 text-center" /><Button variant="danger" size="sm" onClick={() => handleRemoveKitFromOrder(item.id)} className="!p-2" title="Remover">X</Button></div>
                            </div>)
                        })}
                        {order.miscItems.map((item: { componentId: string, quantity: number }, index) => {
                            const component = findComponentById(item.componentId); if (!component) return null;
                             return (<div key={`${item.componentId}-${index}`} className="p-3 bg-white rounded-md shadow-sm border flex justify-between items-center gap-4">
