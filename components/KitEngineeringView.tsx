...
export const KitEngineeringView: React.FC<{ inventory: InventoryHook; manufacturing: ManufacturingHook }> = ({ inventory, manufacturing }) => {
    ...
    const tabItems = [
        ...
        { id: 'inventory', label: 'Inventário & Categorias', icon: <Package size={14} className="mr-2" /> },
    ...
    const validationAlerts = useMemo(() => {
        const alerts: { type: 'warning' | 'error'; message: string; familyId?: string }[] = [];
        ...
        // Check for components without category
        const uncategorized = inventory.components.filter(c => !c.category);
        if (uncategorized.length > 0) {
            alerts.push({ type: 'error', message: `${uncategorized.length} componentes estão sem categoria definida.` });
        }

        return alerts;
    }, [manufacturing.familias, inventory.components]);
    ...
    const globalVariables = useMemo(() => {
        ...
        manufacturing.familias.forEach(f => {
            f.nodes.forEach(n => {
                if (n.data.type === 'processVariable' || n.data.type === 'variableNode') {
                    vars.push({
                        familiaName: f.nome,
                        familiaId: f.id,
                        nodeId: n.id,
        ...
        allItems.forEach(item => {
            ...
            if (!cat) {
                ...
                if (isKit) {
                    ...
                    if (c.sku.includes('PAR-') || c.sku.includes('FIX-') || c.name.toLowerCase().includes('parafuso') || c.name.toLowerCase().includes('fixador')) cat = 'fixadores';
                    else if (c.sku.includes('NUT-') || c.name.toLowerCase().includes('porca') || c.sku.includes('POR-P')) cat = 'porcas';
                ...
                {activeTab === 'dashboard' && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            ...
                            { label: 'Processos Ativos', value: engineeringStats.totalProcesses, icon: <Cpu className="text-emerald-500" /> },
                            { label: 'Regras de Lógica', value: engineeringStats.totalRules, icon: <ShieldCheck className="text-amber-500" /> }
                        ].map((stat, i) => (
                            <Card key={i} className="p-4 border-none shadow-soft flex items-center gap-4">
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        <Card className="border-none shadow-soft p-6">
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">Categorização de Inventário</h3>
                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Arraste itens para organizar o ecossistema</p>
                                </div>
                                <div className="w-64">
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest focus:ring-2 focus:ring-autro-primary/20 outline-none"
                                            value={searchTerm}
                        ...
                        <InventorySlot 
                            title="Fixadores" 
                            categoryId="fixadores"
                            icon={<Settings size={18} />} 
                            items={categories.fixadores} 
                            color="bg-indigo-600" 
                            onDropItem={handleDropItem}
                            onAddItem={() => addToast('Use o menu lateral para adicionar novos componentes específicos.', 'info')}
                        />
                        <InventorySlot 
                            title="Porcas" 
                            categoryId="porcas"
                            icon={<Hexagon size={18} />} 
                            items={categories.porcas} 
                            color="bg-amber-500" 
                            onDropItem={handleDropItem}
                            onAddItem={() => addToast('Use o menu lateral para adicionar novos componentes específicos.', 'info')}
                        />
                        ...
                        <InventorySlot 
                            ...
                            categoryId="tampas"
                            icon={<Layers size={18} />} 
                            items={categories.tampas} 
                            color="bg-blue-600" 
                            onDropItem={handleDropItem}
                            onAddItem={() => addToast('Use o menu lateral para adicionar novos componentes específicos.', 'info')}
                        />
                        <InventorySlot 
                            title="Chaves (Produtos)" 
                            categoryId="chaves"
                            icon={<Wrench size={18} />} 
                            items={categories.chaves} 
                            color="bg-slate-800" 
                            onDropItem={handleDropItem}
                            onAddItem={() => addToast('Use o menu lateral para adicionar novos componentes específicos.', 'info')}
                        />
