...
export const KitEngineeringView: React.FC<{ inventory: InventoryHook; manufacturing: ManufacturingHook }> = ({ inventory, manufacturing }) => {
    ...
    const globalRules = useMemo(() => {
        const rulesMap = new Map<string, { condition: string; result: string }>();
        
        // 1. Start with All Default Rules (Ensures all requested rules are present)
        const defaultRules = [
        ...
        defaultRules.forEach(r => rulesMap.set(`${r.condition}-${r.result}`, r));
        
        // 2. Add rules from all kits (allows user customizations to appear)
        inventory.kits.forEach(k => {
            k.compatibilityRules?.forEach(r => {
                rulesMap.set(`${r.condition}-${r.result}`, r);
            });
        });
