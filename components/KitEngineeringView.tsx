...
export const KitEngineeringView: React.FC<{ inventory: InventoryHook; manufacturing: ManufacturingHook }> = ({ inventory, manufacturing }) => {
    ...
    const validationAlerts = useMemo(() => {
        ...
        if (uncategorized.length > 0) {
            alerts.push({ 
                type: 'error', 
                message: `${uncategorized.length} componentes estão sem categoria definida.`,
                actionType: 'autoCategorize'
            });
        }
                                    ...
                                    validationAlerts.map((alert, i) => (
                                        <div key={i} className={`flex items-start gap-3 p-3 rounded-xl border ${alert.type === 'error' ? 'bg-red-50 border-red-100 text-red-700' : 'bg-amber-50 border-amber-100 text-amber-700'}`}>
                                            ...
                                            <div className="flex-grow">
                                                ...
                                                {alert.actionType === 'autoCategorize' && (
                                                    <button 
                                                        onClick={() => inventory.autoCategorizeAll()}
                                                        className="mt-2 flex items-center gap-1.5 px-3 py-1.5 bg-red-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-red-700 transition-colors shadow-sm"
                                                    >
                                                        <Zap size={10} />
