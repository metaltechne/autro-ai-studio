...
export const useManufacturingOrders = ({ addMultipleInventoryLogs }: ManufacturingOrdersHookProps): ManufacturingOrdersHook => {
    ...
    const addManufacturingOrder = useCallback(async (orderItems: ManufacturingOrderItem[], analysis: ManufacturingAnalysis, extraData?: Partial<ManufacturingOrder>): Promise<string | null> => {
      ...
      const updateManufacturingOrderStatus = useCallback(async (orderId: string, status: ManufacturingOrder['status']) => {
        ...
        if (status === 'concluída' && orderToUpdate.status !== 'concluída') {
            try {
                const movements: Omit<InventoryLog, 'id' | 'date'>[] = [];
                ...
                if (movements.length > 0) {
                    // Centralized call handles both logs and direct stock update
                    await addMultipleInventoryLogs(movements);
                }
            } catch (e) {
    ...
    const finalizeManufacturingItemInspection = useCallback(async (orderId: string, componentId: string, approvedQuantity: number, rejectedQuantity: number) => {
        ...
        const movementsToAdd: Omit<InventoryLog, 'id' | 'date'>[] = [];
        let currentComponents = await api.getComponents();
        
        // Resolve component ID - critical to avoid SKU being used as ID in logs
        let targetComponentId = item.componentId;
        
        // Try to find the component by ID first, then by SKU
        let realComp = currentComponents.find(c => c.id === item.componentId) || currentComponents.find(c => c.sku === item.componentId);
        
        if (!realComp) {
            console.log("Componente não encontrado no estoque, recriando para OF...", item);
            const skuOrigin = item.sku || item.componentId.replace('comp-virtual-', '');
            const sku = skuOrigin.startsWith('comp-') ? skuOrigin.replace('comp-', '') : skuOrigin;
            const sanitizedSku = sku.replace(/[.#$\[\]/]/g, '-');
            
            const newComponentId = `comp-${sanitizedSku}-${Date.now()}`;
            const newComponent = {
                id: newComponentId,
                name: item.name || sku,
                sku: sku,
        ...
        if (movementsToAdd.length > 0) {
            // CENTRALIZED TRANSACTION CALL
            await addMultipleInventoryLogs(movementsToAdd);
        }
