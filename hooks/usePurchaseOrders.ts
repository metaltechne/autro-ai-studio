...
export const usePurchaseOrders = ({ addMultipleInventoryLogs, inventoryHook }: PurchaseOrdersHookProps): PurchaseOrdersHook => {
    ...
    useEffect(() => {
        if (remoteLastModified > lastSync + 2000 && !isLoading) {
            console.log("[PurchaseOrders] Detectada mudança remota. Atualizando...");
            loadData();
        }
    ...
    const addPurchaseOrder = useCallback(async (recommendations: PurchaseRecommendation[], expectedDeliveryDate: string): Promise<string | null> => {
        ...
        const newOrderItems: PurchaseOrderItem[] = recommendations.map(rec => {
            ...
            // If we have specific purchase units recommended, use them and the base purchase price
            if (rec.toOrderPurchaseUnits !== undefined && rec.toOrderPurchaseUnits > 0) {
                return {
                    id: rec.componentId,
                    name: rec.name,
                    quantity: rec.toOrderPurchaseUnits,
                    unitPrice: component?.purchaseCost || 0,
            ...
            return {
                ...
                unitPrice: unitPrice,
                purchaseUnit: rec.consumptionUnit
            };
        });
    ...
    const updateOrderStatus = useCallback(async (orderId: string, status: PurchaseOrder['status']) => {
        ...
        if (orderToUpdate && orderToUpdate.status === 'pendente' && status === 'concluída') {
            const logsToAdd: Omit<InventoryLog, 'id' | 'date'>[] = orderToUpdate.items
                .filter(item => !item.id.startsWith('missing-') && !item.id.startsWith('avulso-') && inventoryHook.components.some(c => c.id === item.id))
                .map(item => {
                    const component = inventoryHook.components.find(c => c.id === item.id);
                    const factor = (component?.purchaseUnit && component?.purchaseQuantity) ? component.purchaseQuantity : 1;
                    const stockQuantity = item.quantity * factor;
                    ...
                    return {
                        ...
                        notes: `Recebimento da Ordem de Compra ${orderId}${factor !== 1 ? ` (${item.quantity} ${item.purchaseUnit || 'un'} x ${factor})` : ''}`,
                    };
                });
            ...
            if (logsToAdd.length > 0) {
                ...
                for (const log of logsToAdd) {
                    await api.updateStockAtomically(log.componentId, log.quantity);
                }
            }
        }
