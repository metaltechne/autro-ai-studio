...
export const useReceiving = (inventoryHook: any) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            const [ordersData, mpsData, c] = await Promise.all([
            ...
            const uniqueOrdersMap = new Map<string, ReceivingOrder>();
            (ordersData || []).forEach(o => { if (o && o.id) uniqueOrdersMap.set(o.id, o); });
            
            const uniqueMappingsMap = new Map<string, SupplierProductMapping>();
            (mpsData || []).forEach(m => { if (m && m.id) uniqueMappingsMap.set(m.id, m); });

            setReceivingOrders(Array.from(uniqueOrdersMap.values()));
            setMappings(Array.from(uniqueMappingsMap.values()));
            setCounter(c);
            setIsLoading(false);
        };
        loadData();
    ...
    const finalizeReceiving = async (orderId: string) => {
        ...
        const movements: { componentId: string, type: 'entrada', quantity: number, reason: InventoryLog['reason'], notes: string }[] = approvedItems.map(item => ({
        ...
        if (movements.length > 0) {
            // USAR O HOOK UNIFICADO: registerBatchStockMovement (ou addMultipleInventoryLogs que agora o usa)
            // Isso já faz o updateStockAtomically E o saveInventoryLogs
            await inventoryHook.addMultipleInventoryLogs(movements);
        }
