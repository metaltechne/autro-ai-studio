...
export const useManufacturing = (): ManufacturingHook => {
    ...
    // Refs for data to prevent save loop
    const familiasRef = useRef<FamiliaComponente[]>([]);
    const workStationsRef = useRef<WorkStation[]>([]);
    const consumablesRef = useRef<Consumable[]>([]);
    const standardOperationsRef = useRef<StandardOperation[]>([]);

    useEffect(() => { savingStatusRef.current = savingStatus; }, [savingStatus]);
    
    // Sync refs
    useEffect(() => { familiasRef.current = familias; }, [familias]);
    useEffect(() => { workStationsRef.current = workStations; }, [workStations]);
    useEffect(() => { consumablesRef.current = consumables; }, [consumables]);
    useEffect(() => { standardOperationsRef.current = standardOperations; }, [standardOperations]);
    ...
    useEffect(() => {
        if (isOutdated && !isDirty && !isLoading) {
            console.log("[Manufacturing] Detectada mudança remota na engenharia. Atualizando...");
            loadData();
        }
    }, [isOutdated, isDirty, isLoading, loadData]);

    // Save drafts to localStorage to prevent data loss
    useEffect(() => {
        if (isDirty) {
    ...
    const saveChanges = useCallback(async () => {
        console.log("[Manufacturing] saveChanges starting.");
        ...
        try {
            ...
            addToast("Alterações sincronizadas com sucesso.", "success");
            setTimeout(() => setSavingStatus('idle'), 2000);
        } catch (error) {
            console.error("Failed to save changes:", error);
            setSavingStatus('idle');
            setIsDirty(true);
            addToast(`Erro ao sincronizar dados com a nuvem: ${error instanceof Error ? error.message : 'Erro desconhecido'}`, "error");
        }
    ...
    useEffect(() => {
        if (isDirty && autoSaveEnabled) {
            const handler = setTimeout(() => {
                saveChanges();
            }, 5000); // 5 seconds of inactivity triggers cloud save

            return () => clearTimeout(handler);
        }
    }, [isDirty, autoSaveEnabled, saveChanges]);

    // PROTEÇÃO: Removido salvamento automático no beforeunload a pedido do usuário
    // O rascunho local já é salvo periodicamente no localStorage pela useEffect abaixo.
    useEffect(() => {
        const handleBeforeUnload = (e: BeforeUnloadEvent) => {
            if (isDirty) {
                // Não salvamos na nuvem automaticamente, apenas avisamos o usuário
                e.preventDefault();
                e.returnValue = '';
            }
        };
    ...
    const updateFamiliaName = useCallback(async (id: string, nome: string) => {
        setFamilias(prev => {
            const next = prev.map(f => f.id === id ? { ...f, nome } : f);
            familiasRef.current = next;
            return next;
        });
        setIsDirty(true);
    }, []);
    ...
    const analyzeManufacturingRun = useCallback((order: ManufacturingOrderItem[], allComponents: Component[], virtualComponents?: Component[], batchHeadCode?: string): ManufacturingAnalysis => {
        ...
        (order || []).forEach(item => {
            ...
            if (item.sourcingOverride === 'purchased') {
                const pCost = component.purchaseCost || 0;
                const buyDivisor = (component.purchaseQuantity && component.purchaseQuantity > 0) ? component.purchaseQuantity : 1;
                const itemTotal = (pCost / buyDivisor) * item.quantity;
                totalCost += itemTotal;
                
                detailedBreakdown.push({
                    name: `${item.quantity}x ${component.name} (Comprado)`,
                    type: 'product',
                    cost: itemTotal,
                ...
                requirements.push({
                    id: component.id,
                    name: component.name,
                    keyName: 'Compra Direta',
                    type: 'inventoryComponent',
                    quantity: item.quantity,
                    unit: 'un',
                    stock: component.stock || 0,
                    shortage: Math.max(0, item.quantity - (component.stock || 0))
                });
                return;
            }
            ...
            (evalRes.costBreakdown || []).forEach(step => {
                // Fix: Only include main nodes (those with nodeId) to avoid duplication with sub-steps (labor/consumables details)
                if (step.type === 'labor' && step.nodeId) {
                    const existingStep = manufacturingSteps.find(ms => ms.nodeId === step.nodeId);
                    if (existingStep) {
                        existingStep.cost += step.cost * item.quantity;
                        existingStep.timeSeconds = (existingStep.timeSeconds || 0) + ((step.timeSeconds || 0) * item.quantity);
                        existingStep.quantity = (existingStep.quantity || 0) + item.quantity;
                    } else {
            ...
            (evalRes.requirements || []).forEach((req: ProcessRequirement) => {
                ...
                const existing = requirements.find(r => r.id === req.id && r.type === req.type);
    ...
    return useMemo(() => ({
        familias, activeFamiliaId, setActiveFamiliaId, getActiveFamilia, addFamilia, updateFamiliaName, deleteFamilia, duplicateFamilia, saveMultipleFamilias: async (f: FamiliaComponente[]) => { 
            setFamilias(f); 
            familiasRef.current = f;
            setIsDirty(true); 
        },
        ...
        updateNodeOperationDetails: (fid: string, nid: string, details: any) => {
            const updatedDetails = { ...details };
            // If selecting a standard operation, clear manual overrides to use operation defaults
            if (details.operationId) {
                updatedDetails.manualTimeSeconds = undefined;
                updatedDetails.manualOperatorId = undefined;
            }
            updateNodeData(fid, nid, updatedDetails);
        },
        ...
        saveWorkStations: async (ws: WorkStation[]) => { 
            setWorkStations(ws); 
            workStationsRef.current = ws;
            setIsDirty(true); 
        },
        saveConsumables: async (c: Consumable[]) => { 
            setConsumables(c); 
            consumablesRef.current = c;
            setIsDirty(true); 
        },
        saveOperations: async (op: StandardOperation[]) => { 
            ...
            standardOperationsRef.current = uniqueOps;
            setIsDirty(true); 
        },
