...
export const useManufacturing = (): ManufacturingHook => {
    ...
    const analyzeManufacturingRun = useCallback((order: ManufacturingOrderItem[], allComponents: Component[], virtualComponents?: Component[], batchHeadCode?: string): ManufacturingAnalysis => {
        ...
        (order || []).forEach(item => {
            ...
            (evalRes.requirements || []).forEach((req: ProcessRequirement) => {
                ...
                const existing = requirements.find(r => r.id === req.id && r.name === req.name);
    ...
    return useMemo(() => ({
        ...
        saveWorkStations: async (ws: WorkStation[]) => { 
            console.log("[Manufacturing] saveWorkStations called, setting isDirty=true");
            setWorkStations(ws); 
            workStationsRef.current = ws;
            setIsDirty(true); 
            // Try to immediately save to avoid autosave race conditions causing data reversion
            try {
                await api.saveWorkStations(ws, true);
                if (autoSaveEnabled) {
                    await api.updateLastModified('engineering');
                    setLastSync(Date.now());
                }
