...
export const usePurchasePlanner = (
    ...
    const abcAnalysisData = useMemo((): Map<string, 'A' | 'B' | 'C'> => {
        ...
        (Array.isArray(inventoryLogs) ? inventoryLogs : Object.values(inventoryLogs || {})).forEach(log => {
            if (log.type === 'saída') {
    ...
    useEffect(() => {
        ...
        // Pre-calculate parsed SKUs for fasteners to avoid repeated regex parsing
        const fastenerDetailsMap = new Map<string, ReturnType<typeof parseFastenerSku>>();
        const fastenerComponents = components.filter(c => {
            if (c.familiaId === 'fam-fixadores') {
                fastenerDetailsMap.set(c.id, parseFastenerSku(c.sku));
                return true;
            }
            return false;
        });
        ...
        while(demandToProcess.size > 0){
            ...
            if(fastenerDetails && component.sourcing !== 'manufactured'){
                const potentialSources = fastenerComponents.filter(c => {
                    ...
                    const sourceDetails = fastenerDetailsMap.get(c.id);
                ...
                }).sort((a,b) => (fastenerDetailsMap.get(a.id)?.comprimento || 0) - (fastenerDetailsMap.get(b.id)?.comprimento || 0));
            ...
            if (component.type === 'raw_material' || component.sourcing === 'purchased') {
                const existingPurchase = purchaseRecs.get(component.id) || {
                    ...
                    purchaseUnit: component.purchaseUnit || 'un',
                    consumptionUnit: component.consumptionUnit || 'un',
                    toOrderPurchaseUnits: 0
                };
                existingPurchase.toOrder += remainingDeficit;
                existingPurchase.required += requiredQty;
                
                // Calculate purchase units respecting rounding strategy
                const factor = component.purchaseQuantity || 1;
                const rawPurchaseUnits = existingPurchase.toOrder / factor;
                
                if (component.purchaseRounding === 'roundUp') {
                    existingPurchase.toOrderPurchaseUnits = Math.ceil(rawPurchaseUnits);
                } else if (component.purchaseRounding === 'exact') {
                    existingPurchase.toOrderPurchaseUnits = rawPurchaseUnits;
                } else {
                    // Smart fallback
                    const pUnitLower = (component.purchaseUnit || '').toLowerCase();
                    const isDiscrete = pUnitLower.includes('barra') || pUnitLower.includes('un') || pUnitLower.includes('pc') || pUnitLower.includes('kit');
                    existingPurchase.toOrderPurchaseUnits = isDiscrete ? Math.ceil(rawPurchaseUnits) : rawPurchaseUnits;
                }
        ...
        rawMaterialNeeds.forEach((requiredQty, materialId) => {
            ...
            if (material) {
                ...
                let estimatedCost = 0;
                let toBuyPurchaseUnits = 0;
                if (netToBuy > 0) {
                    const factor = material.purchaseQuantity || 1;
                    const buyQty = netToBuy / factor;
                    
                    if (material.purchaseRounding === 'roundUp') {
                        toBuyPurchaseUnits = Math.ceil(buyQty);
                    } else if (material.purchaseRounding === 'exact') {
                        toBuyPurchaseUnits = buyQty;
                    } else {
                        const pUnitLower = (material.purchaseUnit || "").toLowerCase();
                        const isDiscrete = pUnitLower.includes("barra") || pUnitLower.includes("un") || pUnitLower.includes("pc") || pUnitLower.includes("kit");
                        toBuyPurchaseUnits = isDiscrete ? Math.ceil(buyQty) : buyQty;
                    }
                    
                    estimatedCost = toBuyPurchaseUnits * (material.purchaseCost || 0);
                }
        ...
        // FINAL SAFETY STEP: Global deduplication to prevent ANY duplicate key errors
        const deduplicate = <T extends { componentId?: string; materialId?: string; targetComponentId?: string }>(list: T[], keyField: keyof T): T[] => {
            const seen = new Set();
        ...
        setPlannerData({
            cuttingRecommendations: deduplicate(Array.from(cuttingRecs.values()), 'targetComponentId'),
            productionPlan: deduplicate(Array.from(productionRecs.values()), 'componentId').sort((a, b) => a.abcClass.localeCompare(b.abcClass)),
            purchasePlan: deduplicate(Array.from(purchaseRecs.values()), 'componentId').sort((a, b) => a.abcClass.localeCompare(b.abcClass)),
            rawMaterialForecast: deduplicate(rawMaterialForecast, 'materialId').sort((a,b) => b.netToBuy - a.netToBuy),
        });
