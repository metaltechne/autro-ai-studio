...
export const calculateKitCosts = (
    ...
    (kit.components || []).forEach(kc => {
        ...
        // Find which family produces this component
        let famId: string | undefined = undefined;
        if (component) {
            // Priority 1: Check if any process flow explicitly designates this component ID as final product
            famId = familias.find(f => 
                f.nodes?.some(n => 
                    (n.data.type === 'finalProduct' || n.data.type === 'final') && 
                    n.data.componentId === component?.id
                )
            )?.id;

            // Priority 2: Check if any process flow generates this SKU via pattern
            if (!famId) {
                famId = familias.find(f => 
                    f.nodes?.some(n => {
                        if (n.data.type !== 'productGenerator' && n.data.type !== 'productGeneratorNode') return false;
                        const template = n.data.generationConfig?.skuTemplate;
                        if (!template) return false;
                        const regexStr = `^${template.replace(/\{[^}]+\}/g, '(.+)')}$`;
                        return new RegExp(regexStr, 'i').test(component?.sku || '');
                    })
                )?.id;
            }
    ...
    if ((isMaterialMapping || isServiceMapping || isSubProcessMapping) && data.dimensions) {
        ...
        if (item) {
            ...
            requirements.push({
                ...
                type: 'inventoryComponent',
                // Resolve dynamic family ID since it was removed from Component interface
                familiaId: config?.allFamilias?.find(f => 
                  f.nodes?.some(n => 
                    ((n.data.type === 'finalProduct' || n.data.type === 'final') && n.data.componentId === item.id) ||
                    ((n.data.type === 'productGenerator' || n.data.type === 'productGeneratorNode') && 
                     n.data.generationConfig?.skuTemplate && 
                     new RegExp(`^${n.data.generationConfig.skuTemplate.replace(/\{[^}]+\}/g, '(.+)')}$`, 'i').test(item.sku))
                  )
                )?.id
            });
