...
export const evaluateProcess = (
    ...
    config?: EvalConfig
): { custoMateriaPrima: number; custoFabricacao: number; totalTimeSeconds: number; nodes: Node<ProcessNodeData>[]; costBreakdown: CostStep[]; requirements: ProcessRequirement[] } => {
    ...
    if (generatorNodes.length === 0) return { custoMateriaPrima: 0, custoFabricacao: 0, totalTimeSeconds: 0, nodes: familia.nodes || [], costBreakdown: [], requirements: [] };
        ...
        if (isGenerator) {
            ...
            (familia.nodes || []).forEach(n => {
                if (individualReaches.has(n.id) && n.id !== node.id && finalActivePath.has(n.id)) {
                    const res = calculateNodeValue(n, allInventoryItems, allStringVariables, allVariables, config, activeDimensionId);
            ...
            (dimensions || []).forEach(dim => {
                ...
                res.nodes.forEach(n => {
                    if (individualReaches.has(n.id) && n.id !== genNode.id) {
                        ...
                        const nodeVal = calculateNodeValue(n, allInventoryItems, sVars, vars, config, dim.id);
