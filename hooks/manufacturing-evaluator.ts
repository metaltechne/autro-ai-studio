...
const calculateNodeValue = (
    ...
    if ((isMaterialMapping || isServiceMapping || isSubProcessMapping) && data.dimensions) {
        ...
        if (!row && allVariables.bitola !== undefined && allVariables.comprimento !== undefined) {
             row = data.dimensions.find(d => {
                const dBitola = String((d as any).bitola || (d as any).Bitola || "").toLowerCase().trim();
                const dCompr = String((d as any).comprimento || (d as any).Comprimento || "").toLowerCase().trim();
                const vBitola = String(allVariables.bitola || "").toLowerCase().trim();
                const vCompr = String(allVariables.comprimento || "").toLowerCase().trim();
                return dBitola === vBitola && dCompr === vCompr;
             });
        }
        ...
        if (!row && allVariables.bitola !== undefined && allVariables.comprimento !== undefined) {
            row = data.dimensions.find(d => {
                ...
                const dBitola = String(d[bitolaKey] || "").toLowerCase().trim();
                const dCompr = comprKey ? String(d[comprKey] || "").toLowerCase().trim() : "0";
                
                const vBitola = String(allVariables.bitola || "").toLowerCase().trim();
                const vCompr = String(allVariables.comprimento || "0").toLowerCase().trim();
        ...
        if (mode === 'fixed') {
            ...
            const consCost = (op?.operationConsumables || []).reduce((sum, oc) => {
                ...
                if (c) {
                    ...
                    return sum + (effectiveQuantity * (c?.unitCost || 0));
                }
                return sum;
            }, 0);
            labor += consCost;
...
export const evaluateProcess = (
    ...
    if (dnaNode?.data.dimensions && dnaNode.data.dimensions.length > 0) {
        ...
        const vBitola = String(allVariables.bitola || (allVariables as any).Bitola || "").toLowerCase().trim();
        const vCompr = String(allVariables.comprimento || (allVariables as any).Comprimento || "").toLowerCase().trim();
        ...
        if (!row && vBitola) {
            row = dnaNode.data.dimensions.find(d => {
                ...
                const db = String(d[bitolaKey] || "").toLowerCase().trim();
                const dc = comprKey ? String(d[comprKey] || "").toLowerCase().trim() : "0";
                
                return db === vBitola && dc === (vCompr || "0");
            });
        }
