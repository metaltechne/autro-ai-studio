...
const getCacheKey = (familia: FamiliaComponente, variables: any, stringVariables: any, config?: EvalConfig) => {
    // Optimized cache key generation - avoid stringifying large structures repeatedly
    const varHash = JSON.stringify(variables) + JSON.stringify(stringVariables);
    const structureHash = `${familia.id}-${familia.nodes?.length}-${familia.edges?.length}`;
    
    // Only include count/presence to avoid massive stringification overhead
    const configHash = config ? `${config.operations?.length}-${config.workStations?.length}-${config.consumables?.length}-${config.allFamilias?.length}` : 'no-config';
