...
export const useInventoryAnalysis = ({
    ...
    const { familias, analyzeManufacturingRun, getAllUniqueHeadCodes, workStations, standardOperations, consumables } = manufacturing;
    ...
    useEffect(() => {
        ...
        aggregatedComponentNeeds.forEach((requiredQty, key) => {
            ...
            if (needsExplosion && familiaId) {
                 ...
                 if (familia) {
                     ...
                     const analysis = evaluateProcess(familia, variables, components, stringVariables, evalConfig);
                     
                     (analysis.requirements || []).forEach(req => {
                         const totalConsumption = req.quantity * requiredQty;
                         const key = req.id || req.sku;
                         materialRequirementsMap.set(key, (materialRequirementsMap.get(key) || 0) + totalConsumption);
                     });
                 }
            }
        });
        ...
        materialRequirementsMap.forEach((required, componentId) => {
            const component = findComponentById(componentId) || findComponentBySku(componentId);
            if (component) {
                ...
                materialRequirementsResult.push({
                    ...
                    name: `Não Encontrado: ${componentId}`,
                    sku: componentId,
                    type: 'raw_material',
                    unit: 'un',
                    currentStock: 0,
