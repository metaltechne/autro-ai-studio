...
export const useCustomers = (): CustomersHook => {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            (data || []).forEach(c => {
                if (c && c.id) uniqueCustomersMap.set(c.id, c);
            });
            const sanitizedData = Array.from(uniqueCustomersMap.values());
            
            setCustomers(sanitizedData.sort((a, b) => a.name.localeCompare(b.name)));
    ...
    useEffect(() => {
        ...
        if (remoteLastModified > lastSync + 2000 && !isLoading) {
            console.log("[Customers] Detectada mudança remota. Atualizando...");
            loadData();
        }
