...
export const useInventory = (): InventoryHook => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
        ...
        if (compDraft || kitDraft || logsDraft) {
            ...
            if (compDraft) setComponents(compDraft.data);
            else setComponents(componentsWithStock);
            
            if (kitDraft) setKits(kitDraft.data);
            else setKits(sanitizedKits);
            
            if (logsDraft) setInventoryLogs(logsDraft.data);
  ...
  // Auto-save logic (Disabled by default)
  useEffect(() => {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
        if (isDirty) {
            // Removido saveChanges() automático aqui a pedido do usuário
            e.preventDefault();
            e.returnValue = '';
        }
    };
        ...
        if (familia.nodes?.some(n => n.data.type === 'productGenerator' || n.data.type === 'productGeneratorNode')) {
            ...
            generatedProducts.forEach(p => {
                ...
                const component: Component = {
                    ...
                    sku: p.sku,
                    stock: 0,
                    type: (p.defaultSourcing === 'purchased' || p.defaultSourcing === 'beneficiado') ? 'raw_material' : 'component',
