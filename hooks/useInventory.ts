...
const analyzeProductionRun = useCallback((
    ...
    order.forEach(orderItem => {
        if (orderItem.type === 'kit') {
            ...
            if (component) {
                aggregatedKitComponents.set(component.sku, (aggregatedKitComponents.get(component.sku) || 0) + orderItem.quantity);
            }
        }
    });
...
const getKits = useCallback(async (): Promise<Kit[]> => {
    ...
    return useMemo(() => ({
        ...
        recalculateAllComponentCosts, createAndStockComponent, getKitsUsingComponent, getKits
    ]);
};
