...
export const useInventory = (): InventoryHook => {
  ...
  const consecutiveRapidSaves = useRef<number>(0);
  ...
  const blockSave = useCallback((reason: string) => {
      setIsBlocked(true);
      addToast(reason, "error");
  }, [addToast]);

  const incrementSaveCount = useCallback(() => {
      // Logic for incrementing internal save tracking if needed
  }, []);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
        ...
        const logs = Array.isArray(logsData) ? logsData : (logsData ? Object.values(logsData) : []);
        ...
        // RESERVED STOCK: Still calculated dynamically from open orders
        ...
        const now = Date.now();
        const componentsWithReserved = uniqueComponents.map(c => ({
            ...c,
            reservedStock: reservedMap.get(c.id) || 0,
        }));

        setComponents(componentsWithReserved);
        setKits(sanitizedKits);
        setInventoryLogs(logs);
  ...
  // Auto-reload when outdated and not dirty
  useEffect(() => {
    if (isOutdated && !isDirty && !isLoading) {
        console.log("[Inventory] Detectada mudança remota. Atualizando dados automaticamente...");
        loadData();
    }
  }, [isOutdated, isDirty, isLoading, loadData]);

  const saveChanges = useCallback(async () => {
    // Inventory is now auto-saved. This exists for UI compatibility.
  ...
  /**
   * REGISTRO PROFISSIONAL DE MOVIMENTAÇÃO (TRANSAÇÃO ATÔMICA)
   * Única via para alteração de estoque. Garante Log + Sincronização Real-time.
   */
  const registerStockMovement = useCallback(async (
      componentId: string, 
      type: 'entrada' | 'saída', 
      quantity: number, 
      ...
      if (!comp || quantity <= 0) return;

      // Proteção de estoque negativo (execeto em ajustes planejados)
      if (type === 'saída' && !['ajuste_inventario_negativo', 'perda_dano'].includes(reason)) {
          if ((comp.stock || 0) < quantity) {
              const msg = `Estoque insuficiente para ${comp.name}. Disponível: ${comp.stock}, Solicitado: ${quantity}`;
              addToast(msg, 'error');
              throw new Error(msg);
          }
      }
      ...
      try {
          // 1. Atualização atômica no servidor
          await api.updateStockAtomically(componentId, delta);
          
          // 2. Persistência do Log
          const currentLogs = await api.getInventoryLogs();
          await api.saveInventoryLogs([...currentLogs, newLog]);

          // 3. Atualização Reativa Local
          setComponents(prev => prev.map(c => 
              c.id === componentId ? { ...c, stock: (c.stock || 0) + delta } : c
          ));
          setInventoryLogs(prev => [newLog, ...prev]);
          
          addToast(`Estoque de ${comp.name} atualizado: ${(comp.stock || 0) + delta}`, 'success');
          await addActivityLog(`Movimentação: ${type} ${quantity}un - ${comp.name} (${reason})`);
      } catch (e) {
          console.error("Erro na transação de estoque:", e);
          addToast("Erro ao sincronizar estoque com o servidor.", "error");
          throw e;
      }
  }, [findComponentById, addToast, addActivityLog]);

  const registerBatchStockMovement = useCallback(async (
    movements: { 
        componentId: string, 
        type: 'entrada' | 'saída', 
        quantity: number, 
    ...
    for (const mov of movements) {
        ...
        updates.set(mov.componentId, (updates.get(mov.componentId) || 0) + delta);
        newLogs.push({ ...mov, id: `log-b-${nanoid(6)}`, date: now });
    }

    try {
        for (const [compId, delta] of Array.from(updates.entries())) {
            await api.updateStockAtomically(compId, delta);
        }
        const currentLogs = await api.getInventoryLogs();
        await api.saveInventoryLogs([...currentLogs, ...newLogs]);

        setComponents(prev => prev.map(c => {
            const d = updates.get(c.id);
            return d ? { ...c, stock: (c.stock || 0) + d } : c;
        }));
        setInventoryLogs(prev => [...newLogs, ...prev]);
        addToast(`${movements.length} movimentações processadas com sucesso.`, 'success');
    } catch (e) {
        console.error("Erro no processamento em lote:", e);
        addToast("Erro ao processar lote no servidor.", "error");
        throw e;
    }
  }, [addToast, findComponentById]);
  ...
  const addComponent = useCallback(async (componentData: Omit<Component, 'id' | 'stock'>): Promise<Component> => {
    ...
    const newComponent: Component = { 
        id: `comp-${sanitizedSku}-${nanoid(6)}`,
        stock: 0, 
        ...componentData,
    } as Component;
    
    const currentComponents = await api.getComponents();
    await api.saveComponents([...currentComponents, newComponent]);

    setComponents(prev => [...prev, newComponent]);
    addToast('Componente cadastrado com sucesso.', 'success');
    return newComponent;
  }, [addToast]);
  
  const updateComponent = useCallback(async (updatedComponent: Component) => {
    const currentComponents = await api.getComponents();
    const newComponents = currentComponents.map(c => c.id === updatedComponent.id ? updatedComponent : c);
  ...
  const updateMultipleComponents = useCallback(async (componentsToUpdate: Component[]) => {
      ...
      addToast(`${componentsToUpdate.length} componentes atualizados com sucesso.`, 'success');
  }, [addToast]);
  ...
  const addMultipleInventoryLogs = useCallback(async (logsData: Omit<InventoryLog, 'id' | 'date'>[]) => {
      return await registerBatchStockMovement(logsData.map(l => ({ ...l, componentId: l.componentId })));
  }, [registerBatchStockMovement]);

  const addInventoryLog = useCallback(async (logData: Omit<InventoryLog, 'id' | 'date'>) => {
      return await registerStockMovement(logData.componentId, logData.type, logData.quantity, logData.reason, logData.notes);
  ...
  const deleteComponent = useCallback(async (componentId: string) => {
      ...
      addToast('Componente removido com sucesso.', 'success');
  }, [addToast]);
  ...
  const deleteKit = useCallback(async (kitId: string) => {
      ...
      addToast('Kit removido com sucesso.', 'success');
  }, [addToast]);
  ...
  const bootstrapInventory = useCallback(async () => {
    if (window.confirm("Isso irá criar uma unidade de cada item. Continuar?")) {
        const logsToAdd = components.map(c => ({
            componentId: c.id,
            type: 'entrada' as const,
            quantity: 1,
            reason: 'ajuste_inventario_positivo' as const,
            notes: 'Bootstrap inicial para testes'
        }));
        await addMultipleInventoryLogs(logsToAdd);
        addToast("Inventário inicializado (1 unidade de cada).", "success");
        loadData();
    }
  }, [components, addMultipleInventoryLogs, addToast, loadData]);
  const recalculateAllComponentCosts = useCallback(async (familias: FamiliaComponente[], allInventoryItems: Component[], auxiliaryData?: { ws: WorkStation[], cons: Consumable[], ops: StandardOperation[], kits: Kit[] }): Promise<SyncReport> => {
        ...
        if (familia.nodes?.some(n => n.data.type === 'productGenerator' || n.data.type === 'productGeneratorNode')) {
            ...
            generatedProducts.forEach(p => {
                const sanitizedSku = p.sku.replace(/[.#$\[\]/]/g, '-');
                    ...
                    sku: p.sku,
                    stock: 0,
                    // Fix: If it has manufacturing cost or is explicitly manufactured, it MUST be a component, not raw_material
                    type: (p.defaultSourcing === 'manufactured' || p.custoFabricacao > 0) ? 'component' : 'raw_material',
    ...
    if (hasChanges) {
        ...
        // Atualizar estado local - sem recalcular estoque pois ele não mudou aqui
...
const getKits = useCallback(async (): Promise<Kit[]> => {
  ...
  return useMemo(() => ({
        ...
        getKits,
        autoCategorizeAll,
        bootstrapInventory, // <--- Aqui
        globalFastenerFamiliaId,
        setGlobalFastenerFamiliaId,
        globalNutFamiliaId,
        setGlobalNutFamiliaId,
    }), [
        ...
        recalculateAllComponentCosts, createAndStockComponent, getKitsUsingComponent, getKits, autoCategorizeAll, bootstrapInventory,
        globalFastenerFamiliaId, setGlobalFastenerFamiliaId, globalNutFamiliaId, setGlobalNutFamiliaId
    ]);
};
