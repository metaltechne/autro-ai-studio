...
const getData = async <T>(key: string, defaultValue: T): Promise<T> => {
    ...
    try {
        // --- SMART CACHING LOGIC ---
        // Verificamos se temos uma versão local cacheada e qual o timestamp de modificação no servidor
        const module = COLLECTION_MODULES[key];
        const lastModifiedKey = module === 'engineering' ? DB_KEYS.lastModified_engineering : 
                               module === 'inventory' ? DB_KEYS.lastModified_inventory : 
                               DB_KEYS.lastModified;
        
        const localLastModified = localStorage.getItem(`cache_ts_${lastModifiedKey}`);
        const localCachedData = localStorage.getItem(`cache_data_${key}`);

        if (localLastModified && localCachedData) {
            // Busca apenas o timestamp do servidor (operação minúscula)
            const serverTsSnapshot = await get(child(dbRef, lastModifiedKey));
            const serverTs = serverTsSnapshot.val();
            
            if (serverTs && serverTs <= JSON.parse(localLastModified)) {
                console.log(`[DataGuard] Cache HIT para ${key}. Usando dados locais para economizar banda.`);
        ...
        // Se o cache falhou ou os dados mudaram, faz o download completo
        ...
        // Pega o timestamp atual do servidor para marcar o cache
        const currentTsSnapshot = await get(child(dbRef, lastModifiedKey));
        const currentTs = currentTsSnapshot.val() || Date.now();
        localStorage.setItem(`cache_ts_${lastModifiedKey}`, JSON.stringify(currentTs));
...
export const updateStockAtomically = async (componentId: string, delta: number): Promise<void> => {
    ...
    const updates: Record<string, any> = {};
    updates[`${DB_KEYS.components}/${sanitizeFirebaseKey(componentId)}/stock`] = increment(delta);
...
export const getComponents = async (): Promise<Component[]> => {
    ...
    const migratedData = rawArray.map((c: any) => {
        ...
        if (initialComp) {
            // Se o SKU ou Nome mudaram no INITIAL_COMPONENTS, força atualização no banco
            if (c.sku !== initialComp.sku || c.name !== initialComp.name) {
                needsUpdate = true;
                return { ...c, ...initialComp, stock: c.stock || 0 };
            }
            
            // Corrige custos se estiverem zerados e houver no inicial
            if ((!c.purchaseCost || c.purchaseCost === 0) && initialComp.purchaseCost) {
                needsUpdate = true;
            ...
            return {
                ...c,
                stock: c.stock || 0,
                custoFabricacao: c.custoFabricacao || (initialComp as any).custoFabricacao || 0,
                custoMateriaPrima: c.custoMateriaPrima || (initialComp as any).custoMateriaPrima || 0
            };
        }
...
export const saveComponents = async (components: Component[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        ...
        if (shouldBlockOperation('write', DB_KEYS.components)) {
            console.warn("[DataGuard] Update Delta para Components bloqueado por segurança.");
            return;
        }
...
export const saveKits = async (kits: Kit[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.kits)) {
            console.warn("[DataGuard] Update Delta para Kits bloqueado por segurança.");
            return;
        }
...
export const getInventoryLogs = async (limit?: number): Promise<InventoryLog[]> => {
    ...
    return getData(DB_KEYS.inventoryLogs, []);
};
export const saveInventoryLogs = async (logs: InventoryLog[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.inventoryLogs)) {
            console.warn("[DataGuard] Update Delta para InventoryLogs bloqueado por segurança.");
            return;
        }
...
export const saveFamilias = async (familias: FamiliaComponente[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.familias)) {
            const msg = "[DataGuard] Gravação de Famílias bloqueada por excesso de frequência.";
            console.warn(msg);
            throw new Error(msg);
        }
...
export const getActivityLogs = async (limit?: number): Promise<ActivityLog[]> => {
    ...
    return getData(DB_KEYS.activityLogs, []);
};
...
export const saveWorkStations = async (data: WorkStation[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.workStations)) {
            const msg = "[DataGuard] Gravação de Workstations bloqueada por excesso de frequência.";
            console.warn(msg);
            throw new Error(msg);
        }
...
export const saveConsumables = async (data: Consumable[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.consumables)) {
            const msg = "[DataGuard] Gravação de Insumos bloqueada por excesso de frequência.";
            console.warn(msg);
            throw new Error(msg);
        }
...
export const saveStandardOperations = async (data: StandardOperation[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.standardOperations)) {
            const msg = "[DataGuard] Gravação de Operações bloqueada por excesso de frequência.";
            console.warn(msg);
            throw new Error(msg);
        }
