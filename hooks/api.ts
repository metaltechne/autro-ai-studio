...
const shouldBlockOperation = (type: 'read' | 'write', key: string): boolean => {
    ...
    // Gatilho 1: Proteção de Loop (mais de 20 gravações na mesma chave em 10 segundos)
    if (type === 'write') {
        const veryRecentWrites = recentHistory.filter(t => t > now - 10000);
        if (veryRecentWrites.length >= 20) {
    ...
    // Gatilho 2: Limite de Frequência (mais de 100 operações por minuto por chave)
...
const saveData = async <T>(key: string, value: T): Promise<void> => {
    ...
    console.log(`[DataGuard Debug] Key: ${key}`);
    console.log(`[DataGuard Debug] Last: ${lastDataCache.get(key)}`);
    console.log(`[DataGuard Debug] New: ${stringified}`);
    ...
    if (lastDataCache.get(key) === stringified) {
        ...
        console.log(`[DataGuard] Cached: ${lastDataCache.get(key)?.slice(0, 50)}...`);
        console.log(`[DataGuard] New: ${stringified.slice(0, 50)}...`);
        return;
    }
