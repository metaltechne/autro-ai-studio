...
import { ref, get, set, remove, child, update, onValue, off, query, orderByChild, orderByKey, limitToFirst, limitToLast, startAt } from '@firebase/database';
...
export const saveWorkStations = async (data: WorkStation[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.workStations)) {
            console.warn("[DataGuard] Update Delta para Workstations bloqueado por segurança.");
            return;
        }
...
export const saveConsumables = async (data: Consumable[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.consumables)) {
            console.warn("[DataGuard] Update Delta para Consumables bloqueado por segurança.");
            return;
        }
...
export const saveStandardOperations = async (data: StandardOperation[], skipTimestamp = false): Promise<void> => {
    ...
    if (hasChanges) {
        if (shouldBlockOperation('write', DB_KEYS.standardOperations)) {
            console.warn("[DataGuard] Update Delta para StandardOperations bloqueado por segurança.");
            return;
        }
