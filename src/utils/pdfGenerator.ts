    ...
    try {
        ...
        if (taxDetails && taxDetails.length > 0) {
            taxDetails.forEach(tax => {
                doc.text(`${tax.name}: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(tax.value)}`, 200, currentY, { align: 'right' });
                currentY += 5;
            });
        } else if (taxValue > 0) {
            doc.text(`Impostos: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(taxValue)}`, 200, currentY, { align: 'right' });
            currentY += 5;
        }

        if (shippingValue > 0) {
            doc.text(`Frete: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(shippingValue)}`, 200, currentY, { align: 'right' });
            currentY += 5;
        }
        ...
        doc.setTextColor(40);
        doc.setFont('helvetica', 'bold');
        doc.text(`Valor Total: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalValue)}`, 200, currentY + 5, { align: 'right' });
        ...
        doc.save(`Orcamento_${clientName.replace(/\s+/g, '_')}_${new Date().getTime()}.pdf`);
        return true;
    } catch (error) {
