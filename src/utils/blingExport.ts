import { ProductionOrder, Customer, Kit, InventoryHook, Deal, Lead } from '../../types';

export const exportToBlingCSV = (
    data: (ProductionOrder | Deal)[],
    customers: (Customer | Lead)[],
    ...
    data.forEach(item => {
        const isOrder = 'orderItems' in item;
        const customerId = isOrder ? (item as ProductionOrder).customerId : (item as Deal).leadId;
        ...
        const items = isOrder ? (item as ProductionOrder).orderItems : (item as Deal).items || [];
    ...
    if (data.length === 1) {
        const item = data[0];
        const isOrder = 'orderItems' in item;
        const customerId = isOrder ? (item as any).customerId : (item as any).leadId;
        const customer = customers.find(c => c.id === customerId);
