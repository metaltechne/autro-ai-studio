...
export const KitsByBrandView: React.FC<KitsByBrandViewProps> = ({ inventory, manufacturing }) => {
  ...
  const fastenerFamilia = useMemo(() => {
    if (settings.preferredFastenerFamiliaId) {
      const found = familias.find(f => f.id === settings.preferredFastenerFamiliaId);
      if (found) return found;
    }
    return familias.find(f => f.id === 'fam-fixadores') || 
           familias.find(f => f.masterProcessTag === 'FIX-S') || 
           familias.find(f => f.masterProcessTag === 'FIX-P');
  }, [familias, settings.preferredFastenerFamiliaId]);
  ...
  const kitCostsAndSales = useMemo(() => {
    ...
    return costMap;
  }, [kits, familias, components, inventory.components, calculateSaleDetails, fastenerFamilia]);
  ...
  }, [filteredKits, familias, components, inventory.components, fastenerFamilia]);
